...
pub struct ParseData {
    ...
    pub(crate) indent_tracker: Option<IndentTracker>,
}

impl ParseData {
    pub fn new(frame_stack: FrameStack) -> Self {
        Self { frame_stack: Some(frame_stack), indent_tracker: Some(IndentTracker::default()) }
    }
    ...
    pub fn forward(&self, other: Self) -> Self {
        let frame_stack = match (&self.frame_stack, other.frame_stack) {
            (Some(frame_stack1), Some(frame_stack2)) => Some(frame_stack1 | frame_stack2),
            (None, None) => None,
            _ => panic!()
        };
        Self { frame_stack, indent_tracker: None }
