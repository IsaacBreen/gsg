...
%token LAMBDA DEF ELSE DOUBLESLASH PASS SLASHEQUAL FSTRING_MIDDLE AS DOUBLESLASHEQUAL BREAK CIRCUMFLEX NOTEQUAL RIGHTSHIFT PLUSEQUAL STAR PERCENT LBRACE LSQB EXCLAMATION VBAR LESS TYPE_COMMENT DEDENT RAISE EQUAL GLOBAL GREATEREQUAL AMPER RPAR WHILE SLASH NONLOCAL EQEQUAL IMPORT NONE MINUS VBAREQUAL _NAME MATCH MINEQUAL INDENT CONTINUE CIRCUMFLEXEQUAL RARROW DOUBLESTAREQUAL NEWLINE GREATER COLONEQUAL WITH DOUBLESTAR FINALLY CASE YIELD FOR ASYNC IN ATEQUAL RSQB COLON NOT FSTRING_START IF IS LESSEQUAL ENDMARKER LEFTSHIFTEQUAL PLUS AND ELLIPSIS OR LPAR NUMBER COMMA CLASS ELIF FROM EXCEPT RIGHTSHIFTEQUAL RETURN STRING TRY TRUE UNDERSCORE SEMI TILDE FALSE AMPEREQUAL AWAIT DEL LEFTSHIFT AT TYPE PERCENTEQUAL ASSERT DOT STAREQUAL FSTRING_END RBRACE
