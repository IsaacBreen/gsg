...
pub fn precompute<'a>(
    ...
    for state_id in 0..tokenizer.max_state() {
        let mut state_map: BTreeMap<Vec<TokenID>, BTreeMap<&'a [u8], StateID>> = BTreeMap::new();
...
mod tests {
    ...
    #[test]
    fn test_simple_precompute() {
        // Manually define the DFA
        let tokenizer = Regex {
            dfa: DFA {
                states: vec![
                    // State 0
                    DFAState {
                        transitions: TrieMap::from_iter(vec![
                            (97, 1), // 'a' -> State 1
                            (98, 2), // 'b' -> State 2
                        ]),
                    ...
                    DFAState {
                        ...
                        finalizers: BTreeSet::from([0]), // Finalizes Token 0 ('a')
                    },
                    // State 2
                    DFAState {
                        transitions: TrieMap::new(),
                        finalizers: BTreeSet::from([2]), // Finalizes Token 2 ('b')
                    },
                    // State 3
                    DFAState {
                        transitions: TrieMap::new(),
                        finalizers: BTreeSet::from([1]), // Finalizes Token 1 ('ab')
                    },
                ],
                start_state: 0,
            },
        ...
        let llm_tokens: &[&[u8]] = &[
            ...
            b"ab",   // LLM Token 1
            b"b",    // LLM Token 2
            b"abc",  // LLM Token 3 (Invalid)
        ];

        // Expected precompute output
        // For each state, map grammar token sequences to LLM tokens that can produce them and their resulting states
        let mut expected: BTreeMap<StateID, BTreeMap<Vec<TokenID>, BTreeMap<&[u8], StateID>>> = BTreeMap::new();

        // State 0
        expected.insert(0, {
            let mut state_map = BTreeMap::new();
            // LLM Token 0: 'a' -> Token 0 ('a') leads to State 1
            state_map.insert(vec![0], BTreeMap::from([ (b"a".as_slice(), 1) ]));

            // LLM Token 1: 'ab' -> Token 1 ('ab') leads to State 3
            state_map.insert(vec![1], BTreeMap::from([ (b"ab".as_slice(), 3) ]));

            // LLM Token 2: 'b' -> Token 2 ('b') leads to State 2
            state_map.insert(vec![2], BTreeMap::from([ (b"b".as_slice(), 2) ]));

            // LLM Token 3: 'abc' -> Invalid (no matching token sequences)
            // Thus, it doesn't appear in the mapping
        ...
        expected.insert(1, {
            let mut state_map = BTreeMap::new();
            // LLM Token 0: 'a' -> No valid transition from State 1 with 'a'
            // LLM Token 1: 'ab' -> 'a' consumed leads to no transition, invalid
            // LLM Token 2: 'b' -> Token 0 ('a') followed by 'b' -> Token 1 ('ab') leads to State 3
            // LLM Token 3: 'abc' -> 'a' then 'b' then 'c' -> 'ab' consumed leads to State 3, 'c' invalid
            state_map.insert(vec![1], BTreeMap::from([ (b"b".as_slice(), 3) ]));
            // Note: LLM Token 3 partially matches 'ab' and then 'c' is invalid. Depending on implementation, it might map to Token 1 with remaining 'c'.
            state_map
        });
        ...
        expected.insert(2, {
            let mut state_map = BTreeMap::new();
            // From State 2, no transitions. Any LLM token would be invalid.
            // Thus, no entries.
            state_map
        });
        ...
        expected.insert(3, {
            let mut state_map = BTreeMap::new();
            // From State 3, no transitions. Any LLM token would be invalid.
            state_map
        });

        // Define the expected LLM tokens mapping per state
        // Now, run precompute
        let actual = precompute(&tokenizer, llm_tokens);
