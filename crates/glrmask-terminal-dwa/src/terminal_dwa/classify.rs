...
/// Classify one token using the canonical regular-language partition.
///
/// The historical implementation of this API was an arbitrary Rust decision
/// tree. That implementation is retained only as a test oracle below; the
/// production source of truth is now the compiled regular partition spec, so
/// the exact same partition language is available to containment analysis.
pub fn classify_vocab_char_type(bytes: &[u8]) -> u8 {
    // Optimized evaluator for the canonical regular partition. The language
    // definition lives in `regular_partition`; exhaustive regression tests and
    // the optional full-vocabulary verifier assert this evaluator is identical
    // to the DFA semantics. Keeping the evaluator avoids determinizing the
    // Unicode partition merely to route vocabulary tokens.
...
mod tests {
    ...
    use super::{
        classify_terminal_path_lengths, classify_vocab_char_type, fast_eval_char_type_regular_partition,
