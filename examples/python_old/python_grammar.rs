use crate::seq;
use crate::IntoDyn;
use crate::{cache_context, cached, deferred, eat_string, forbid_follows, forbid_follows_check_not, forbid_follows_clear, lookahead, mutate_right_data, negative_lookahead, tag, CombinatorTrait, RightData, RightDataGetters};
...
pub fn python_literal(s: &str) -> impl CombinatorTrait {
    let increment_scope_count = |right_data: &mut RightData| { right_data.get_fields1_mut().scope_count += 1; true };
    let decrement_scope_count = |right_data: &mut RightData| { right_data.get_fields1_mut().scope_count -= 1; true };
