...
fn build_vocab_partition_direct_mask_tokenizer(
    ...
    let proxy_ms = elapsed_ms(proxy_started);
    ...
    let prepare_components_ms = elapsed_ms(prepare_started);
    let Some(prepared) = prepared else {
        return Ok(None);
    };
    let horizon_started = Instant::now();
    repeat_horizons.prewarm_dfas(prepared.iter().map(|prepared| prepared.body_dfa()), vocab);
    let horizon_prewarm_ms = elapsed_ms(horizon_started);
    ...
    let unique_components = prepared
        ...
        .collect::<Option<Vec<_>>>();
    let finite_components_ms = elapsed_ms(finite_started);
    let components_ms = elapsed_ms(components_started);
    ...
    if compile_profile_enabled() {
        eprintln!(
            "[glrmask/profile][vocab_partition_tokenizer] path=direct_mask states={} components={} preflight_ms={preflight_ms:.3} expressions_ms={expressions_ms:.3} scan_ms={scan_ms:.3} proxy_ms={proxy_ms:.3} prepare_components_ms={prepare_components_ms:.3} horizon_prewarm_ms={horizon_prewarm_ms:.3} finite_components_ms={finite_components_ms:.3} components_ms={components_ms:.3} install_ms={install_ms:.3} total_ms={:.3}",
            tokenizer.num_states(),
            bounded_code_terminals.len(),
            elapsed_ms(total_started),
        );
