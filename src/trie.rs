...
impl<T, E: Ord> TrieNode<E, T> {
    ...
    pub fn insert(&mut self, edge: E, child: Arc<Mutex<TrieNode<E, T>>>) -> Option<Arc<Mutex<TrieNode<E, T>>>> {
        ...
        self.children.insert(edge, child).map(|existing_child| {
    ...
    pub fn deep_clone(&self) -> Arc<Mutex<TrieNode<E, T>>>
    where
        T: Clone,
        E: Clone,
    {
        let children = self
            .children
            .iter()
            .map(|(edge, child)| (edge.clone(), child.lock().unwrap().deep_clone()))
            .collect();
    ...
    pub fn shallow_clone(&self) -> Arc<Mutex<TrieNode<E, T>>>
    where
        T: Clone,
        E: Clone,
    {
    ...
    pub fn replace_child_with_clone(&mut self, edge: &E) -> Arc<Mutex<TrieNode<E, T>>>
    where
        T: Clone,
        E: Clone,
    {
    ...
    pub fn all_nodes(root: Arc<Mutex<TrieNode<E, T>>>) -> Vec<Arc<Mutex<TrieNode<E, T>>>> {
        ...
        while let Some(node) = queue.pop_front() {
            ...
            for (_, child) in &node.lock().unwrap().children {
...
impl<T: Clone, E: Ord + Clone> TrieNode<E, T> {
        ...
        E: Ord,
    {
        ...
        if !dormant_states.is_empty() {
            for (node_ptr, _) in &dormant_states {
