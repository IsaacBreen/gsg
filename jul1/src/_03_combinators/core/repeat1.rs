            ...
            match parse_result {
                Ok(one_shot_up_data) => {
                    if !self.greedy && prev_parse_result.is_ok() {
                        ...
                        parse_results_rest.up_data_vec.push(UpData::new(prev_right_data, one_shot_up_data.output));
                ...
                Err(UnambiguousParseError::Fail) => {
                    if let Ok(one_shot_up_data) = prev_parse_result {
                        return (Repeat1Parser {
                            ...
                            greedy: self.greedy
                        }, ParseResults::new_single(UpData::new(one_shot_up_data.just_right_data(), one_shot_up_data.output), true));
                    } else {
                        return (Repeat1Parser {
                            a: &self.a,
                            a_parsers: vec![],
                ...
                Err(UnambiguousParseError::Ambiguous) => {
                    ...
                    if let Ok(one_shot_up_data) = prev_parse_result {
                        parse_results_rest.up_data_vec.push(UpData::new(one_shot_up_data.just_right_data(), one_shot_up_data.output));
                    }
                    return (parser, parse_results_rest);
                }
...
impl<'a, T> ParserTrait<Vec<T::Output>> for Repeat1Parser<'a, T> where T: CombinatorTrait {
    ...
    fn parse<'b>(&mut self, bytes: &'b [u8]) -> ParseResults<Vec<T::Output>> where Vec<T::Output>: 'b {
