     ...
     fn compile(mut self) -> Self {
        ...
        fn compile_inner(combinator: &dyn CombinatorTrait, deferred_cache: &mut HashMap<usize, Ref>) {
            if let Some(deferred) = combinator.as_any().downcast_ref::<Deferred>() {
                let new_inner: DeferredInner = match deferred.inner.borrow().deref() {
                    ...
                    DeferredInner::CompiledWeak(weak) => {
                        if let Some(cached) = deferred_cache.get(&deferred.f.get_addr()) {
                            cached.clone().into()
                        } else {
                            let strong = strong_ref();
                            let weak = strong.downgrade();
                            deferred_cache.insert(deferred.f.get_addr(), Ref::Weak(weak.clone()));
                            let mut evaluated: Combinator = deferred.f.evaluate_deferred_fn_to_combinator();
                            compile_inner(&mut evaluated, deferred_cache);
                            dbg!(&evaluated);
                            strong.set(evaluated);
                            deferred_cache.insert(deferred.f.get_addr(), Ref::Strong(strong.clone()));
                            DeferredInner::CompiledStrong(strong.clone())
                        }
                    }
                };
                *deferred.inner.borrow_mut() = new_inner;
                // dbg!(&inner);
            } else {
