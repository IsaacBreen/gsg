...
pub fn assert_parses_fast<T: CombinatorTrait, S: ToString>(combinator: &T, input: S) {
    ...
    let max_position = parse_results.right_data_vec.iter().max_by_key(|right_data| right_data.right_data_inner.borrow().position).expect(format!("Expected at least one right data. parse_results: {:?}", parse_results).as_str()).right_data_inner.borrow().position;
    ...
    assert!(parse_results.right_data_vec.iter().max_by_key(|right_data| right_data.right_data_inner.borrow().position).expect(format!("Expected at least one right data. parse_results: {:?}", parse_results).as_str()).right_data_inner.borrow().position == bytes.len(), "Expected parser to finish with right data at the end position {}. parse_results: {:?}", bytes.len(), parse_results);
...
pub fn assert_fails_fast<T: CombinatorTrait, S: ToString>(combinator: &T, input: S) {
    ...
    assert!(parse_results.done && parse_results.right_data_vec.iter().max_by_key(|right_data| right_data.right_data_inner.borrow().position).map_or(true, |right_data| right_data.right_data_inner.borrow().position == bytes.len()), "Expected parser to fail at the end. parse_results: {:?}", parse_results);
}