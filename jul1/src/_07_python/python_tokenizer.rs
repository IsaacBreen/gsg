...
use crate::{check_right_data, choice_fast, choice_greedy, eat_bytestring_choice,
            ...
            eat_string, eps, exclude_strings, fail,
            mutate_right_data, negative_lookahead,
...
pub fn whitespace()-> impl CombinatorTrait {
    choice_greedy!(
        ...
        seq!(
            check_right_data(|right_data| right_data.get_fields1().scope_count > 0),
            repeat1_fast(eat_string_choice_fast(&[" ", "\t", "\\\n", "\n", "\r"]))
        ),
    )
}
...
pub fn FSTRING_MIDDLE()-> impl CombinatorTrait {
    ...
    choice_greedy!(
        seq!(
            check_right_data(|right_data| { *right_data.get_fields2().fstring_start_stack.last().unwrap() == PythonQuoteType::OneSingle }),
            one_single,
        ),
        seq!(
            check_right_data(|right_data| { *right_data.get_fields2().fstring_start_stack.last().unwrap() == PythonQuoteType::OneDouble }),
            one_double,
        ),
        seq!(
            check_right_data(|right_data| { matches!(right_data.get_fields2().fstring_start_stack.last().unwrap(), PythonQuoteType::ThreeSingle | PythonQuoteType::ThreeDouble) }),
            three,
        ),
    )
}
