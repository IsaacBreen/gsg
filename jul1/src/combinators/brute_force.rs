...
impl CombinatorTrait for BruteForce {
    ...
    fn parser(&self, right_data: RightData) -> (Self::Parser, Vec<RightData>, Vec<UpData>) {
        let ParseResults { right_data_vec: right_data2, up_data_vec: up_data } = (self.f)(&Vec::new(), &right_data);
...
impl ParserTrait for BruteForceParser {
    fn step(&mut self, c: u8) -> ParseResults {
        self.values.push(c);
        let ParseResults { right_data_vec: right_data2, up_data_vec: up_data } = (self.f)(&self.values, &self.right_data);
        ParseResults {
