...
pub fn opt(a: impl Into<Combinator>) -> Opt<Combinator> {
    Opt { inner: a.into(), greedy: false }
}
...
impl<T: CombinatorTrait + 'static> From<Opt<T>> for Combinator {
    fn from(value: Opt<T>) -> Self {
        Combinator::Dyn(Box::new(value))
    }
}
