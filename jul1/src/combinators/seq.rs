...
use crate::{Combinator, CombinatorTrait, eps, Parser, ParseResults, ParserTrait, RightData, UpData};
...
impl CombinatorTrait for Seq {
    fn parser(&self, right_data: RightData) -> (Parser, ParseResults) {
        ...
        // }
        let children = self.children.iter().map(|child| (child.clone(), Vec::new())).collect();
        let parser = Parser::SeqParser(SeqParser { children });

        for (combinator, parsers) in &mut self.children {
            helper(c, combinator, parsers, &mut current_right_data, &mut all_up_data, &mut all_done);
        }
...
fn helper(c: u8, combinator: &Combinator, parsers: &mut Vec<Parser>, current_right_data: &mut Vec<RightData>, all_up_data: &mut Vec<UpData>, all_done: &mut bool) {
    ...
    for right_data in current_right_data.drain(..) {
