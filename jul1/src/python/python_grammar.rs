...
pub fn python_file() -> Combinator {
    ...
    let WS = symbol(tag("WS", seq!(forbid_follows_check_not(Forbidden::WS as usize), WS(), forbid_follows(&[Forbidden::DEDENT as usize, Forbidden::INDENT as usize, Forbidden::NEWLINE as usize]))));
    let NAME = symbol(tag("NAME", seq!(forbid_follows_check_not(Forbidden::NAME as usize), NAME(), forbid_follows(&[Forbidden::NAME as usize, Forbidden::NUMBER as usize]))));
    let TYPE_COMMENT = symbol(tag("TYPE_COMMENT", seq!(TYPE_COMMENT())));
    let FSTRING_START = symbol(tag("FSTRING_START", seq!(FSTRING_START(), forbid_follows(&[Forbidden::WS as usize]))));
    let FSTRING_MIDDLE = symbol(tag("FSTRING_MIDDLE", seq!(forbid_follows_check_not(Forbidden::FSTRING_MIDDLE as usize), FSTRING_MIDDLE(), forbid_follows(&[Forbidden::FSTRING_MIDDLE as usize, Forbidden::WS as usize]))));
    let FSTRING_END = symbol(tag("FSTRING_END", seq!(FSTRING_END())));
    let NUMBER = symbol(tag("NUMBER", seq!(forbid_follows_check_not(Forbidden::NUMBER as usize), NUMBER(), forbid_follows(&[Forbidden::NUMBER as usize]))));
    let STRING = symbol(tag("STRING", seq!(STRING())));
    let NEWLINE = symbol(tag("NEWLINE", seq!(forbid_follows_check_not(Forbidden::NEWLINE as usize), NEWLINE(), forbid_follows(&[Forbidden::WS as usize]))));
    let INDENT = symbol(tag("INDENT", seq!(forbid_follows_check_not(Forbidden::INDENT as usize), INDENT(), forbid_follows(&[Forbidden::WS as usize]))));
    let DEDENT = symbol(tag("DEDENT", seq!(forbid_follows_check_not(Forbidden::DEDENT as usize), DEDENT(), forbid_follows(&[Forbidden::WS as usize]))));
    let ENDMARKER = symbol(tag("ENDMARKER", seq!(ENDMARKER())));
