...
use crate::{cache_context, cached, choice, Choice, CombinatorTrait, DynCombinator, eat_char_choice, eat_char_range, eat_string, eps, Eps, forbid_follows, forbid_follows_check_not, forbid_follows_clear, forward_decls, forward_ref, IntoCombinator, opt, Repeat1, seprep0, seprep1, Seq, tag};
use crate::{repeat0, repeat1, seq};

use super::python_tokenizer::{DEDENT, ENDMARKER, FSTRING_END, FSTRING_MIDDLE, FSTRING_START, INDENT, NAME, NEWLINE, NUMBER, STRING, TYPE_COMMENT, WS};
use super::python_tokenizer::python_literal;

pub fn python_file() -> Rc<DynCombinator> {
    let WS = tag("WS", seq(forbid_follows_check_not("WS"), WS(), forbid_follows(&["DEDENT","INDENT","NEWLINE"]))).into_rc_dyn();
    let NAME = tag("NAME", seq(forbid_follows_check_not("NAME"), NAME(), forbid_follows(&["NAME","NUMBER"]))).into_rc_dyn();
    let TYPE_COMMENT = tag("TYPE_COMMENT", seq(forbid_follows_check_not("TYPE_COMMENT"), TYPE_COMMENT(), forbid_follows(&[]))).into_rc_dyn();
    let FSTRING_START = tag("FSTRING_START", seq(forbid_follows_check_not("FSTRING_START"), FSTRING_START(), forbid_follows(&["WS"]))).into_rc_dyn();
    let FSTRING_MIDDLE = tag("FSTRING_MIDDLE", seq(forbid_follows_check_not("FSTRING_MIDDLE"), FSTRING_MIDDLE(), forbid_follows(&["FSTRING_MIDDLE","WS"]))).into_rc_dyn();
    let FSTRING_END = tag("FSTRING_END", seq(forbid_follows_check_not("FSTRING_END"), FSTRING_END(), forbid_follows(&[]))).into_rc_dyn();
    let NUMBER = tag("NUMBER", seq(forbid_follows_check_not("NUMBER"), NUMBER(), forbid_follows(&["NUMBER"]))).into_rc_dyn();
    let STRING = tag("STRING", seq(forbid_follows_check_not("STRING"), STRING(), forbid_follows(&[]))).into_rc_dyn();
    let NEWLINE = tag("NEWLINE", seq(forbid_follows_check_not("NEWLINE"), NEWLINE(), forbid_follows(&["WS"]))).into_rc_dyn();
    let INDENT = tag("INDENT", seq(forbid_follows_check_not("INDENT"), INDENT(), forbid_follows(&["WS"]))).into_rc_dyn();
    let DEDENT = tag("DEDENT", seq(forbid_follows_check_not("DEDENT"), DEDENT(), forbid_follows(&["WS"]))).into_rc_dyn();
    let ENDMARKER = tag("ENDMARKER", seq(forbid_follows_check_not("ENDMARKER"), ENDMARKER(), forbid_follows(&[]))).into_rc_dyn();
