...
pub fn python_file() -> Rc<DynCombinator> {
    let WS = tag("WS", seq!(prevent_consecutive_matches_check_not("WS"), WS(), prevent_consecutive_matches(&["DEDENT","NEWLINE"]))).into_rc_dyn();
    ...
    let FSTRING_MIDDLE = tag("FSTRING_MIDDLE", seq!(prevent_consecutive_matches_check_not("FSTRING_MIDDLE"), FSTRING_MIDDLE(), prevent_consecutive_matches(&["WS"]))).into_rc_dyn();
