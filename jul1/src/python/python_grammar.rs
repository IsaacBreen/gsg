use std::rc::Rc;
use crate::{choice, opt, eat_char_choice, eat_string, eat_char_range, forward_ref, eps, cut, tag, cached, cache_context, lookahead, prevent_consecutive_matches, prevent_consecutive_matches_check_not, DynCombinator, CombinatorTrait, forward_decls, seprep0, seprep1, IntoCombinator, Seq2, Choice2, Repeat1, Eps};
...
pub fn python_file() -> Rc<DynCombinator> {
    ...
    let FSTRING_START = tag("FSTRING_START", seq!(FSTRING_START(), prevent_consecutive_matches(&["WS"]))).into_rc_dyn();
    ...
    let NEWLINE = tag("NEWLINE", seq!(NEWLINE(), prevent_consecutive_matches(&["WS"]))).into_rc_dyn();
    let INDENT = tag("INDENT", seq!(INDENT(), prevent_consecutive_matches(&["WS"]))).into_rc_dyn();
    let DEDENT = tag("DEDENT", seq!(DEDENT(), prevent_consecutive_matches(&["WS"]))).into_rc_dyn();
