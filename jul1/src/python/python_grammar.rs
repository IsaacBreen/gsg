...
fn WS() -> Combinator { cached(tag("WS", crate::profile("WS", seq!(forbid_follows_clear(), token::WS().compile())))).into() }
...
fn FSTRING_START() -> Combinator { cached(tag("FSTRING_START", crate::profile("FSTRING_START", seq!(forbid_follows_clear(), token::FSTRING_START().compile())))).into() }
fn FSTRING_MIDDLE() -> Combinator { cached(tag("FSTRING_MIDDLE", crate::profile("FSTRING_MIDDLE", seq!(forbid_follows_check_not(Forbidden::FSTRING_MIDDLE as usize), token::FSTRING_MIDDLE().compile(), forbid_follows(&[Forbidden::FSTRING_MIDDLE as usize]))))).into() }
...
fn NEWLINE() -> Combinator { cached(tag("NEWLINE", crate::profile("NEWLINE", seq!(forbid_follows_clear(), token::NEWLINE().compile())))).into() }
fn INDENT() -> Combinator { cached(tag("INDENT", crate::profile("INDENT", seq!(forbid_follows_clear(), token::INDENT().compile())))).into() }
fn DEDENT() -> Combinator { cached(tag("DEDENT", crate::profile("DEDENT", seq!(forbid_follows_clear(), token::DEDENT().compile())))).into() }
...
fn del_targets() -> Combinator {
    tag("del_targets", seq!(&del_target, opt(repeat1(seq!(python_literal(","), &del_target))), opt(python_literal(",")))).into()
}
...
fn star_targets_list_seq() -> Combinator {
    tag("star_targets_list_seq", seq!(&star_target, opt(repeat1(seq!(python_literal(","), &star_target))), opt(python_literal(",")))).into()
}
...
fn args() -> Combinator {
    tag("args", choice!(
        seq!(choice!(&starred_expression, seq!(choice!(seq!(&NAME, python_literal(":="), &expression), seq!(choice!(seq!(&disjunction, opt(seq!(python_literal("if"), &disjunction, python_literal("else"), &expression))), &lambdef), negative_lookahead(python_literal(":=")))), negative_lookahead(python_literal("=")))), opt(repeat1(seq!(python_literal(","), choice!(&starred_expression, seq!(choice!(seq!(&NAME, python_literal(":="), &expression), seq!(choice!(seq!(&disjunction, opt(seq!(python_literal("if"), &disjunction, python_literal("else"), &expression))), &lambdef), negative_lookahead(python_literal(":=")))), negative_lookahead(python_literal("="))))))), opt(seq!(python_literal(","), &kwargs))),
        &kwargs
    )).into()
}
...
fn double_starred_kvpairs() -> Combinator {
    tag("double_starred_kvpairs", seq!(&double_starred_kvpair, opt(repeat1(seq!(python_literal(","), &double_starred_kvpair))), opt(python_literal(",")))).into()
}
...
fn star_named_expressions() -> Combinator {
    tag("star_named_expressions", seq!(&star_named_expression, opt(repeat1(seq!(python_literal(","), &star_named_expression))), opt(python_literal(",")))).into()
}
...
fn type_param_seq() -> Combinator {
    tag("type_param_seq", seq!(&type_param, opt(repeat1(seq!(python_literal(","), &type_param))), opt(python_literal(",")))).into()
}
...
fn positional_patterns() -> Combinator {
    tag("positional_patterns", seq!(choice!(&as_pattern, &or_pattern), opt(repeat1(seq!(python_literal(","), &pattern))))).into()
}
...
fn maybe_sequence_pattern() -> Combinator {
    tag("maybe_sequence_pattern", seq!(&maybe_star_pattern, opt(repeat1(seq!(python_literal(","), &maybe_star_pattern))), opt(python_literal(",")))).into()
}
...
fn decorators() -> Combinator {
    tag("decorators", seq!(python_literal("@"), &named_expression, &NEWLINE, opt(repeat1(seq!(python_literal("@"), &named_expression, &NEWLINE))))).into()
}

fn block() -> Combinator {
    cached(tag("block", choice!(
        seq!(&NEWLINE, &INDENT, &statements, &DEDENT),
        seq!(&simple_stmt, choice!(seq!(negative_lookahead(python_literal(";")), &NEWLINE), seq!(opt(repeat1(seq!(python_literal(";"), &simple_stmt))), opt(python_literal(";")), &NEWLINE)))
    ))).into()
}
...
fn nonlocal_stmt() -> Combinator {
    tag("nonlocal_stmt", seq!(python_literal("nonlocal"), &NAME, opt(repeat1(seq!(python_literal(","), &NAME))))).into()
}

fn global_stmt() -> Combinator {
    tag("global_stmt", seq!(python_literal("global"), &NAME, opt(repeat1(seq!(python_literal(","), &NAME))))).into()
}
...
fn assignment() -> Combinator {
    tag("assignment", choice!(
        ...
        seq!(&star_targets, python_literal("="), opt(repeat1(seq!(&star_targets, python_literal("=")))), choice!(&yield_expr, &star_expressions), negative_lookahead(python_literal("=")), opt(&TYPE_COMMENT)),
...
fn simple_stmts() -> Combinator {
    tag("simple_stmts", seq!(&simple_stmt, choice!(seq!(negative_lookahead(python_literal(";")), &NEWLINE), seq!(opt(repeat1(seq!(python_literal(";"), &simple_stmt))), opt(python_literal(";")), &NEWLINE)))).into()
}
