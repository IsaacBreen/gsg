use std::rc::Rc;
use crate::{cache_first_context, cache_first, lookahead_context, symbol, Symbol, Choice, deferred, Combinator, CombinatorTrait, eat_char_choice, eat_char_range, eat_string, eps, Eps, forbid_follows, forbid_follows_check_not, forbid_follows_clear, forward_decls, forward_ref, Repeat1, Seq, tag, Compile, lookahead, negative_lookahead, profile, fail, eat};
use super::python_tokenizer::python_literal;
use crate::{seq, cached, cache_context};
...
pub fn python_file() -> Combinator {
    ...
    let WS = symbol(fail());
    let NAME = symbol(eat("x"));
    let TYPE_COMMENT = symbol(fail());
    let FSTRING_START = symbol(fail());
    let FSTRING_MIDDLE = symbol(fail());
    let FSTRING_END = symbol(fail());
    let NUMBER = symbol(fail());
    let STRING = symbol(fail());
    let NEWLINE = symbol(eat("\n"));
    let INDENT = symbol(fail());
    let DEDENT = symbol(fail());
    let ENDMARKER = symbol(fail());
    ...
    let star_targets = star_targets.set(tag("star_targets", seq!(&star_target, choice!(negative_lookahead(seq!(opt(&WS), python_literal(","))), seq!(opt(seq!(opt(&WS), python_literal(","), opt(&WS), &star_target, opt(repeat1(seq!(opt(&WS), python_literal(","), opt(&WS), &star_target))))), opt(seq!(opt(&WS), python_literal(","))))))));
    ...
    let lambda_param_maybe_default = lambda_param_maybe_default.set(tag("lambda_param_maybe_default", seq!(&lambda_param, opt(seq!(opt(&WS), &default)), choice!(seq!(opt(&WS), python_literal(",")), lookahead(seq!(opt(&WS), python_literal(":")))))));
    let lambda_param_with_default = lambda_param_with_default.set(tag("lambda_param_with_default", seq!(&lambda_param, opt(&WS), &default, choice!(seq!(opt(&WS), python_literal(",")), lookahead(seq!(opt(&WS), python_literal(":")))))));
    let lambda_param_no_default = lambda_param_no_default.set(tag("lambda_param_no_default", seq!(&lambda_param, choice!(seq!(opt(&WS), python_literal(",")), lookahead(seq!(opt(&WS), python_literal(":")))))));
    ...
    let value_pattern = value_pattern.set(tag("value_pattern", seq!(&attr, negative_lookahead(seq!(opt(&WS), choice!(python_literal("."), python_literal("("), python_literal("=")))))));
    let wildcard_pattern = wildcard_pattern.set(tag("wildcard_pattern", python_literal("_")));
    let pattern_capture_target = pattern_capture_target.set(tag("pattern_capture_target", seq!(negative_lookahead(seq!(python_literal("_"), opt(&WS))), &NAME, negative_lookahead(seq!(opt(&WS), choice!(python_literal("."), python_literal("("), python_literal("=")))))));
    ...
    let with_item = with_item.set(tag("with_item", seq!(&expression, opt(seq!(opt(&WS), python_literal("as"), opt(&WS), &star_target, lookahead(seq!(opt(&WS), choice!(python_literal(","), python_literal(")"), python_literal(":")))))))));
    ...
    let param_maybe_default = param_maybe_default.set(tag("param_maybe_default", seq!(&param, opt(seq!(opt(&WS), &default)), choice!(seq!(opt(&WS), python_literal(","), opt(seq!(opt(&WS), &TYPE_COMMENT))), seq!(opt(seq!(opt(&WS), &TYPE_COMMENT)), lookahead(seq!(opt(&WS), python_literal(")"))))))));
    let param_with_default = param_with_default.set(tag("param_with_default", seq!(&param, opt(&WS), &default, choice!(seq!(opt(&WS), python_literal(","), opt(seq!(opt(&WS), &TYPE_COMMENT))), seq!(opt(seq!(opt(&WS), &TYPE_COMMENT)), lookahead(seq!(opt(&WS), python_literal(")"))))))));
    let param_no_default_star_annotation = param_no_default_star_annotation.set(tag("param_no_default_star_annotation", seq!(&param_star_annotation, choice!(seq!(opt(&WS), python_literal(","), opt(seq!(opt(&WS), &TYPE_COMMENT))), seq!(opt(seq!(opt(&WS), &TYPE_COMMENT)), lookahead(seq!(opt(&WS), python_literal(")"))))))));
    let param_no_default = param_no_default.set(tag("param_no_default", seq!(&param, choice!(seq!(opt(&WS), python_literal(","), opt(seq!(opt(&WS), &TYPE_COMMENT))), seq!(opt(seq!(opt(&WS), &TYPE_COMMENT)), lookahead(seq!(opt(&WS), python_literal(")"))))))));
    ...
    let del_stmt = del_stmt.set(tag("del_stmt", seq!(python_literal("del"), opt(&WS), &del_targets, lookahead(seq!(opt(&WS), choice!(python_literal(";"), &NEWLINE))))));
    ...
    let simple_stmts = simple_stmts.set(tag("simple_stmts", seq!(&simple_stmt, opt(&WS), choice!(seq!(negative_lookahead(seq!(python_literal(";"), opt(&WS))), &NEWLINE), seq!(opt(seq!(python_literal(";"), opt(&WS), &simple_stmt, opt(repeat1(seq!(opt(&WS), python_literal(";"), opt(&WS), &simple_stmt))), opt(&WS))), opt(seq!(python_literal(";"), opt(&WS))), &NEWLINE)))));
