...
pub fn python_file() -> Combinator {
    ...
    let WS = symbol(cached(cache_first(tag("WS", seq!(forbid_follows_check_not(Forbidden::WS as usize), token::WS().compile(), forbid_follows(&[Forbidden::DEDENT as usize, Forbidden::INDENT as usize, Forbidden::NEWLINE as usize]))))));
    let NAME = symbol(cached(cache_first(tag("NAME", seq!(forbid_follows_check_not(Forbidden::NAME as usize), token::NAME().compile(), forbid_follows(&[Forbidden::NAME as usize, Forbidden::NUMBER as usize]))))));
    let TYPE_COMMENT = symbol(cached(cache_first(tag("TYPE_COMMENT", seq!(forbid_follows_clear(), token::TYPE_COMMENT().compile())))));
    let FSTRING_START = symbol(cached(cache_first(tag("FSTRING_START", seq!(token::FSTRING_START().compile(), forbid_follows(&[Forbidden::WS as usize]))))));
    let FSTRING_MIDDLE = symbol(cached(cache_first(tag("FSTRING_MIDDLE", seq!(forbid_follows_check_not(Forbidden::FSTRING_MIDDLE as usize), token::FSTRING_MIDDLE().compile(), forbid_follows(&[Forbidden::FSTRING_MIDDLE as usize, Forbidden::WS as usize]))))));
    let FSTRING_END = symbol(cached(cache_first(tag("FSTRING_END", seq!(forbid_follows_clear(), token::FSTRING_END().compile())))));
    let NUMBER = symbol(cached(cache_first(tag("NUMBER", seq!(forbid_follows_check_not(Forbidden::NUMBER as usize), token::NUMBER().compile(), forbid_follows(&[Forbidden::NUMBER as usize]))))));
    let STRING = symbol(cached(cache_first(tag("STRING", seq!(forbid_follows_clear(), token::STRING().compile())))));
    let NEWLINE = symbol(cached(cache_first(tag("NEWLINE", seq!(forbid_follows_check_not(Forbidden::NEWLINE as usize), token::NEWLINE().compile(), forbid_follows(&[Forbidden::WS as usize]))))));
    let INDENT = symbol(cached(cache_first(tag("INDENT", seq!(forbid_follows_check_not(Forbidden::INDENT as usize), token::INDENT().compile(), forbid_follows(&[Forbidden::WS as usize]))))));
    let DEDENT = symbol(cached(cache_first(tag("DEDENT", seq!(forbid_follows_check_not(Forbidden::DEDENT as usize), token::DEDENT().compile(), forbid_follows(&[Forbidden::WS as usize]))))));
    let ENDMARKER = symbol(cached(cache_first(tag("ENDMARKER", seq!(forbid_follows_clear(), token::ENDMARKER().compile())))));
