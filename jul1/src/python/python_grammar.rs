use std::rc::Rc;
use crate::{choice, opt, eat_char_choice, eat_string, eat_char_range, forward_ref, eps, tag, cached, cache_context, forbid_consecutive_matches, forbid_consecutive_matches_clear, forbid_consecutive_matches_check_not, DynCombinator, CombinatorTrait, forward_decls, seprep0, seprep1, IntoCombinator, Seq2, Choice2, Repeat1, Eps};
...
pub fn python_file() -> Rc<DynCombinator> {
    let WS = tag("WS", seq!(forbid_consecutive_matches_check_not("WS"), WS(), forbid_consecutive_matches(&["DEDENT","INDENT","NEWLINE"]))).into_rc_dyn();
    let NAME = tag("NAME", seq!(forbid_consecutive_matches_check_not("NAME"), NAME(), forbid_consecutive_matches(&["NAME","NUMBER"]))).into_rc_dyn();
    let TYPE_COMMENT = tag("TYPE_COMMENT", seq!(forbid_consecutive_matches_check_not("TYPE_COMMENT"), TYPE_COMMENT(), forbid_consecutive_matches(&[]))).into_rc_dyn();
    let FSTRING_START = tag("FSTRING_START", seq!(forbid_consecutive_matches_check_not("FSTRING_START"), FSTRING_START(), forbid_consecutive_matches(&["WS"]))).into_rc_dyn();
    let FSTRING_MIDDLE = tag("FSTRING_MIDDLE", seq!(forbid_consecutive_matches_check_not("FSTRING_MIDDLE"), FSTRING_MIDDLE(), forbid_consecutive_matches(&["FSTRING_MIDDLE","WS"]))).into_rc_dyn();
    let FSTRING_END = tag("FSTRING_END", seq!(forbid_consecutive_matches_check_not("FSTRING_END"), FSTRING_END(), forbid_consecutive_matches(&[]))).into_rc_dyn();
    let NUMBER = tag("NUMBER", seq!(forbid_consecutive_matches_check_not("NUMBER"), NUMBER(), forbid_consecutive_matches(&["NUMBER"]))).into_rc_dyn();
    let STRING = tag("STRING", seq!(forbid_consecutive_matches_check_not("STRING"), STRING(), forbid_consecutive_matches(&[]))).into_rc_dyn();
    let NEWLINE = tag("NEWLINE", seq!(forbid_consecutive_matches_check_not("NEWLINE"), NEWLINE(), forbid_consecutive_matches(&["WS"]))).into_rc_dyn();
    let INDENT = tag("INDENT", seq!(forbid_consecutive_matches_check_not("INDENT"), INDENT(), forbid_consecutive_matches(&["WS"]))).into_rc_dyn();
    let DEDENT = tag("DEDENT", seq!(forbid_consecutive_matches_check_not("DEDENT"), DEDENT(), forbid_consecutive_matches(&["WS"]))).into_rc_dyn();
    let ENDMARKER = tag("ENDMARKER", seq!(forbid_consecutive_matches_check_not("ENDMARKER"), ENDMARKER(), forbid_consecutive_matches(&[]))).into_rc_dyn();
    ...
    let statements = statements.set(tag("statements", seq!(&statement, opt(repeat1(seq!(opt(&WS), &statement)))))).into_rc_dyn();
    ...
    let eval = eval.set(tag("eval", seq!(&expressions, opt(seq!(opt(&WS), &NEWLINE, opt(repeat1(seq!(opt(&WS), &NEWLINE))))), opt(&WS), &ENDMARKER))).into_rc_dyn();
