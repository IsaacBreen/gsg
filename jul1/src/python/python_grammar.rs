...
pub fn python_file() -> Rc<DynCombinator> {
    ...
    let expression_without_invalid = expression_without_invalid.set(tag("expression_without_invalid", choice!(
        seq!(&conjunction, opt(seq!(opt(&WS), python_literal("or"), opt(&WS), opt(seq!(&WS, opt(&WS))), &conjunction, opt(seq!(opt(&WS), python_literal("or"), opt(&WS), opt(seq!(&WS, opt(&WS))), &conjunction)))), opt(seq!(opt(&WS), python_literal("if"), opt(&WS), &disjunction, opt(&WS), python_literal("else"), opt(&WS), &expression))),
    ...
    let del_target = del_target.set(tag("del_target", choice!(
        seq!(choice!(&NAME, python_literal("True"), python_literal("False"), python_literal("None"), &strings, &NUMBER, &tuple, &group, &genexp, &list, &listcomp, &dict, &set, &dictcomp, &setcomp, python_literal("...")), opt(seq!(opt(&WS), choice!(seq!(python_literal("."), opt(&WS), opt(seq!(&WS, opt(&WS))), &NAME), seq!(python_literal("["), opt(&WS), opt(seq!(&WS, opt(&WS))), &slices, opt(&WS), opt(seq!(&WS, opt(&WS))), python_literal("]")), &genexp, seq!(python_literal("("), opt(seq!(opt(&WS), opt(seq!(&WS, opt(&WS))), &arguments)), opt(&WS), opt(seq!(&WS, opt(&WS))), python_literal(")"))), opt(seq!(opt(&WS), choice!(seq!(python_literal("."), opt(&WS), opt(seq!(&WS, opt(&WS))), &NAME), seq!(python_literal("["), opt(&WS), opt(seq!(&WS, opt(&WS))), &slices, opt(&WS), opt(seq!(&WS, opt(&WS))), python_literal("]")), &genexp, seq!(python_literal("("), opt(seq!(opt(&WS), opt(seq!(&WS, opt(&WS))), &arguments)), opt(&WS), opt(seq!(&WS, opt(&WS))), python_literal(")"))))))), opt(&WS), choice!(seq!(python_literal("."), opt(&WS), &NAME), seq!(python_literal("["), opt(&WS), &slices, opt(&WS), python_literal("]")))),
        &del_t_atom
    ))).into_rc_dyn();
    let del_targets = del_targets.set(tag("del_targets", seq!(&del_target, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &del_target, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &del_target)))), opt(seq!(opt(&WS), python_literal(",")))))).into_rc_dyn();
    let t_lookahead = t_lookahead.set(tag("t_lookahead", choice!(
        python_literal("("),
        python_literal("["),
        python_literal(".")
    ))).into_rc_dyn();
    let t_primary = t_primary.set(tag("t_primary", seq!(choice!(&NAME, python_literal("True"), python_literal("False"), python_literal("None"), &strings, &NUMBER, &tuple, &group, &genexp, &list, &listcomp, &dict, &set, &dictcomp, &setcomp, python_literal("...")), opt(seq!(opt(&WS), choice!(seq!(python_literal("."), opt(&WS), &NAME), seq!(python_literal("["), opt(&WS), &slices, opt(&WS), python_literal("]")), &genexp, seq!(python_literal("("), opt(seq!(opt(&WS), &arguments)), opt(&WS), python_literal(")"))), opt(seq!(opt(&WS), choice!(seq!(python_literal("."), opt(&WS), &NAME), seq!(python_literal("["), opt(&WS), &slices, opt(&WS), python_literal("]")), &genexp, seq!(python_literal("("), opt(seq!(opt(&WS), &arguments)), opt(&WS), python_literal(")")))))))))).into_rc_dyn();
    ...
    let star_targets_tuple_seq = star_targets_tuple_seq.set(tag("star_targets_tuple_seq", seq!(&star_target, opt(&WS), python_literal(","), opt(seq!(opt(&WS), &star_target, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &star_target)), opt(seq!(opt(&WS), python_literal(",")))))))).into_rc_dyn();
    let star_targets_list_seq = star_targets_list_seq.set(tag("star_targets_list_seq", seq!(&star_target, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &star_target, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &star_target)))), opt(seq!(opt(&WS), python_literal(",")))))).into_rc_dyn();
    let star_targets = star_targets.set(tag("star_targets", seq!(&star_target, opt(seq!(opt(seq!(opt(&WS), python_literal(","), opt(&WS), &star_target, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &star_target)))), opt(seq!(opt(&WS), python_literal(",")))))))).into_rc_dyn();
    ...
    let kwargs = kwargs.set(tag("kwargs", choice!(
        seq!(&kwarg_or_starred, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &kwarg_or_starred, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &kwarg_or_starred)))), opt(seq!(opt(&WS), python_literal(","), opt(&WS), &kwarg_or_double_starred, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &kwarg_or_double_starred, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &kwarg_or_double_starred))))))),
        seq!(&kwarg_or_double_starred, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &kwarg_or_double_starred, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &kwarg_or_double_starred)))))
    ))).into_rc_dyn();
    ...
    let for_if_clause = for_if_clause.set(tag("for_if_clause", choice!(
        seq!(python_literal("async"), opt(&WS), python_literal("for"), opt(&WS), &star_targets, opt(&WS), python_literal("in"), opt(&WS), &disjunction, opt(seq!(opt(&WS), python_literal("if"), opt(&WS), &disjunction, opt(seq!(opt(&WS), python_literal("if"), opt(&WS), &disjunction))))),
        seq!(python_literal("for"), opt(&WS), &star_targets, opt(&WS), python_literal("in"), opt(&WS), &disjunction, opt(seq!(opt(&WS), python_literal("if"), opt(&WS), &disjunction, opt(seq!(opt(&WS), python_literal("if"), opt(&WS), &disjunction)))))
    ))).into_rc_dyn();
    let for_if_clauses = for_if_clauses.set(tag("for_if_clauses", seq!(&for_if_clause, opt(seq!(opt(&WS), &for_if_clause))))).into_rc_dyn();
    ...
    let double_starred_kvpairs = double_starred_kvpairs.set(tag("double_starred_kvpairs", seq!(&double_starred_kvpair, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &double_starred_kvpair, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &double_starred_kvpair)))), opt(seq!(opt(&WS), python_literal(",")))))).into_rc_dyn();
    ...
    let strings = strings.set(tag("strings", seq!(choice!(seq!(&FSTRING_START, opt(seq!(&fstring_middle, opt(seq!(opt(&WS), &fstring_middle)))), &FSTRING_END), &STRING), opt(seq!(opt(&WS), choice!(seq!(&FSTRING_START, opt(seq!(&fstring_middle, opt(seq!(opt(&WS), &fstring_middle)))), &FSTRING_END), &STRING)))))).into_rc_dyn();
    let string = string.set(tag("string", &STRING)).into_rc_dyn();
    let fstring = fstring.set(tag("fstring", seq!(&FSTRING_START, opt(seq!(&fstring_middle, opt(seq!(opt(&WS), &fstring_middle)))), &FSTRING_END))).into_rc_dyn();
    ...
    let fstring_full_format_spec = fstring_full_format_spec.set(tag("fstring_full_format_spec", seq!(python_literal(":"), opt(seq!(opt(&WS), &fstring_format_spec, opt(seq!(opt(&WS), &fstring_format_spec))))))).into_rc_dyn();
    ...
    let lambda_star_etc = lambda_star_etc.set(tag("lambda_star_etc", choice!(
        seq!(python_literal("*"), opt(&WS), choice!(seq!(&lambda_param_no_default, opt(seq!(opt(&WS), &lambda_param_maybe_default, opt(seq!(opt(&WS), &lambda_param_maybe_default)))), opt(seq!(opt(&WS), &lambda_kwds))), seq!(python_literal(","), opt(&WS), &lambda_param_maybe_default, opt(seq!(opt(&WS), &lambda_param_maybe_default)), opt(seq!(opt(&WS), &lambda_kwds))))),
        &lambda_kwds
    ))).into_rc_dyn();
    let lambda_slash_with_default = lambda_slash_with_default.set(tag("lambda_slash_with_default", seq!(opt(seq!(&lambda_param_no_default, opt(seq!(opt(&WS), &lambda_param_no_default)), opt(&WS))), &lambda_param_with_default, opt(seq!(opt(&WS), &lambda_param_with_default)), opt(&WS), python_literal("/"), opt(seq!(opt(&WS), python_literal(",")))))).into_rc_dyn();
    let lambda_slash_no_default = lambda_slash_no_default.set(tag("lambda_slash_no_default", seq!(&lambda_param_no_default, opt(seq!(opt(&WS), &lambda_param_no_default)), opt(&WS), python_literal("/"), opt(seq!(opt(&WS), python_literal(",")))))).into_rc_dyn();
    let lambda_parameters = lambda_parameters.set(tag("lambda_parameters", choice!(
        seq!(&lambda_slash_no_default, opt(seq!(opt(&WS), &lambda_param_no_default, opt(seq!(opt(&WS), &lambda_param_no_default)))), opt(seq!(opt(&WS), &lambda_param_with_default, opt(seq!(opt(&WS), &lambda_param_with_default)))), opt(seq!(opt(&WS), &lambda_star_etc))),
        seq!(&lambda_slash_with_default, opt(seq!(opt(&WS), &lambda_param_with_default, opt(seq!(opt(&WS), &lambda_param_with_default)))), opt(seq!(opt(&WS), &lambda_star_etc))),
        seq!(&lambda_param_no_default, opt(seq!(opt(&WS), &lambda_param_no_default)), opt(seq!(opt(&WS), &lambda_param_with_default, opt(seq!(opt(&WS), &lambda_param_with_default)))), opt(seq!(opt(&WS), &lambda_star_etc))),
        seq!(&lambda_param_with_default, opt(seq!(opt(&WS), &lambda_param_with_default)), opt(seq!(opt(&WS), &lambda_star_etc))),
        &lambda_star_etc
    ))).into_rc_dyn();
    ...
    let slices = slices.set(tag("slices", choice!(
        &slice,
        seq!(choice!(&slice, &starred_expression), opt(seq!(opt(&WS), python_literal(","), opt(&WS), choice!(&slice, &starred_expression), opt(seq!(opt(&WS), python_literal(","), opt(&WS), choice!(&slice, &starred_expression))))), opt(seq!(opt(&WS), python_literal(","))))
    ))).into_rc_dyn();
    let primary = primary.set(tag("primary", seq!(&atom, opt(seq!(opt(&WS), choice!(seq!(python_literal("."), opt(&WS), &NAME), &genexp, seq!(python_literal("("), opt(seq!(opt(&WS), &arguments)), opt(&WS), python_literal(")")), seq!(python_literal("["), opt(&WS), &slices, opt(&WS), python_literal("]"))), opt(seq!(opt(&WS), choice!(seq!(python_literal("."), opt(&WS), &NAME), &genexp, seq!(python_literal("("), opt(seq!(opt(&WS), &arguments)), opt(&WS), python_literal(")")), seq!(python_literal("["), opt(&WS), &slices, opt(&WS), python_literal("]")))))))))).into_rc_dyn();
    ...
    let term = term.set(tag("term", seq!(&factor, opt(seq!(opt(&WS), choice!(seq!(python_literal("*"), opt(&WS), &factor), seq!(python_literal("/"), opt(&WS), &factor), seq!(python_literal("//"), opt(&WS), &factor), seq!(python_literal("%"), opt(&WS), &factor), seq!(python_literal("@"), opt(&WS), &factor)), opt(seq!(opt(&WS), choice!(seq!(python_literal("*"), opt(&WS), &factor), seq!(python_literal("/"), opt(&WS), &factor), seq!(python_literal("//"), opt(&WS), &factor), seq!(python_literal("%"), opt(&WS), &factor), seq!(python_literal("@"), opt(&WS), &factor))))))))).into_rc_dyn();
    let sum = sum.set(tag("sum", seq!(&term, opt(seq!(opt(&WS), choice!(seq!(python_literal("+"), opt(&WS), &term), seq!(python_literal("-"), opt(&WS), &term)), opt(seq!(opt(&WS), choice!(seq!(python_literal("+"), opt(&WS), &term), seq!(python_literal("-"), opt(&WS), &term))))))))).into_rc_dyn();
    let shift_expr = shift_expr.set(tag("shift_expr", seq!(&sum, opt(seq!(opt(&WS), choice!(seq!(python_literal("<<"), opt(&WS), &sum), seq!(python_literal(">>"), opt(&WS), &sum)), opt(seq!(opt(&WS), choice!(seq!(python_literal("<<"), opt(&WS), &sum), seq!(python_literal(">>"), opt(&WS), &sum))))))))).into_rc_dyn();
    let bitwise_and = bitwise_and.set(tag("bitwise_and", seq!(&shift_expr, opt(seq!(opt(&WS), python_literal("&"), opt(&WS), &shift_expr, opt(seq!(opt(&WS), python_literal("&"), opt(&WS), &shift_expr))))))).into_rc_dyn();
    let bitwise_xor = bitwise_xor.set(tag("bitwise_xor", seq!(&bitwise_and, opt(seq!(opt(&WS), python_literal("^"), opt(&WS), &bitwise_and, opt(seq!(opt(&WS), python_literal("^"), opt(&WS), &bitwise_and))))))).into_rc_dyn();
    let bitwise_or = bitwise_or.set(tag("bitwise_or", seq!(&bitwise_xor, opt(seq!(opt(&WS), python_literal("|"), opt(&WS), &bitwise_xor, opt(seq!(opt(&WS), python_literal("|"), opt(&WS), &bitwise_xor))))))).into_rc_dyn();
    ...
    let comparison = comparison.set(tag("comparison", seq!(&bitwise_or, opt(seq!(opt(&WS), &compare_op_bitwise_or_pair, opt(seq!(opt(&WS), &compare_op_bitwise_or_pair))))))).into_rc_dyn();
    let inversion = inversion.set(tag("inversion", choice!(
        seq!(python_literal("not"), opt(&WS), &inversion),
        &comparison
    ))).into_rc_dyn();
    let conjunction = conjunction.set(tag("conjunction", seq!(&inversion, opt(seq!(opt(&WS), python_literal("and"), opt(&WS), &inversion, opt(seq!(opt(&WS), python_literal("and"), opt(&WS), &inversion))))))).into_rc_dyn();
    let disjunction = disjunction.set(tag("disjunction", seq!(&conjunction, opt(seq!(opt(&WS), python_literal("or"), opt(&WS), &conjunction, opt(seq!(opt(&WS), python_literal("or"), opt(&WS), &conjunction))))))).into_rc_dyn();
    ...
    let star_named_expressions = star_named_expressions.set(tag("star_named_expressions", seq!(&star_named_expression, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &star_named_expression, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &star_named_expression)))), opt(seq!(opt(&WS), python_literal(",")))))).into_rc_dyn();
    let star_expression = star_expression.set(tag("star_expression", choice!(
        seq!(python_literal("*"), opt(&WS), &bitwise_or),
        seq!(&disjunction, opt(seq!(opt(&WS), python_literal("if"), opt(&WS), &disjunction, opt(&WS), python_literal("else"), opt(&WS), &expression))),
        &lambdef
    ))).into_rc_dyn();
    let star_expressions = star_expressions.set(tag("star_expressions", seq!(&star_expression, opt(seq!(opt(&WS), python_literal(","), opt(seq!(opt(&WS), &star_expression, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &star_expression)), opt(seq!(opt(&WS), python_literal(",")))))))))).into_rc_dyn();
    ...
    let expressions = expressions.set(tag("expressions", seq!(&expression, opt(seq!(opt(&WS), python_literal(","), opt(seq!(opt(&WS), &expression, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &expression)), opt(seq!(opt(&WS), python_literal(",")))))))))).into_rc_dyn();
    ...
    let type_param_seq = type_param_seq.set(tag("type_param_seq", seq!(&type_param, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &type_param, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &type_param)))), opt(seq!(opt(&WS), python_literal(",")))))).into_rc_dyn();
    ...
    let keyword_patterns = keyword_patterns.set(tag("keyword_patterns", seq!(&keyword_pattern, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &keyword_pattern, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &keyword_pattern))))))).into_rc_dyn();
    let positional_patterns = positional_patterns.set(tag("positional_patterns", seq!(choice!(&as_pattern, &or_pattern), opt(seq!(opt(&WS), python_literal(","), opt(&WS), &pattern, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &pattern))))))).into_rc_dyn();
    let class_pattern = class_pattern.set(tag("class_pattern", seq!(&NAME, opt(seq!(opt(&WS), python_literal("."), opt(&WS), opt(seq!(&WS, opt(&WS))), &NAME, opt(seq!(opt(&WS), python_literal("."), opt(&WS), opt(seq!(&WS, opt(&WS))), &NAME)))), opt(&WS), python_literal("("), opt(&WS), choice!(python_literal(")"), seq!(&positional_patterns, opt(&WS), choice!(seq!(opt(seq!(python_literal(","), opt(&WS))), python_literal(")")), seq!(python_literal(","), opt(&WS), &keyword_patterns, opt(seq!(opt(&WS), python_literal(","))), opt(&WS), python_literal(")")))), seq!(&keyword_patterns, opt(seq!(opt(&WS), python_literal(","))), opt(&WS), python_literal(")")))))).into_rc_dyn();
    ...
    let items_pattern = items_pattern.set(tag("items_pattern", seq!(&key_value_pattern, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &key_value_pattern, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &key_value_pattern))))))).into_rc_dyn();
    ...
    let maybe_sequence_pattern = maybe_sequence_pattern.set(tag("maybe_sequence_pattern", seq!(&maybe_star_pattern, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &maybe_star_pattern, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &maybe_star_pattern)))), opt(seq!(opt(&WS), python_literal(",")))))).into_rc_dyn();
    ...
    let name_or_attr = name_or_attr.set(tag("name_or_attr", seq!(&NAME, opt(seq!(opt(&WS), python_literal("."), opt(&WS), &NAME, opt(seq!(opt(&WS), python_literal("."), opt(&WS), &NAME))))))).into_rc_dyn();
    ...
    let or_pattern = or_pattern.set(tag("or_pattern", seq!(&closed_pattern, opt(seq!(opt(&WS), python_literal("|"), opt(&WS), &closed_pattern, opt(seq!(opt(&WS), python_literal("|"), opt(&WS), &closed_pattern))))))).into_rc_dyn();
    ...
    let match_stmt = match_stmt.set(tag("match_stmt", seq!(python_literal("match"), opt(&WS), &subject_expr, opt(&WS), python_literal(":"), opt(&WS), &NEWLINE, opt(&WS), &INDENT, opt(&WS), &case_block, opt(seq!(opt(&WS), &case_block)), opt(&WS), &DEDENT))).into_rc_dyn();
    ...
    let try_stmt = try_stmt.set(tag("try_stmt", seq!(python_literal("try"), opt(&WS), python_literal(":"), opt(&WS), &block, opt(&WS), choice!(&finally_block, seq!(&except_block, opt(seq!(opt(&WS), &except_block)), opt(seq!(opt(&WS), &else_block)), opt(seq!(opt(&WS), &finally_block))), seq!(&except_star_block, opt(seq!(opt(&WS), &except_star_block)), opt(seq!(opt(&WS), &else_block)), opt(seq!(opt(&WS), &finally_block))))))).into_rc_dyn();
    ...
    let with_stmt = with_stmt.set(tag("with_stmt", choice!(
        seq!(python_literal("with"), opt(&WS), choice!(seq!(python_literal("("), opt(&WS), &with_item, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &with_item, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &with_item)))), opt(seq!(opt(&WS), python_literal(","))), opt(&WS), python_literal(")"), opt(&WS), python_literal(":"), opt(seq!(opt(&WS), &TYPE_COMMENT)), opt(&WS), &block), seq!(&with_item, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &with_item, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &with_item)))), opt(&WS), python_literal(":"), opt(seq!(opt(&WS), &TYPE_COMMENT)), opt(&WS), &block))),
        seq!(python_literal("async"), opt(&WS), python_literal("with"), opt(&WS), choice!(seq!(python_literal("("), opt(&WS), &with_item, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &with_item, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &with_item)))), opt(seq!(opt(&WS), python_literal(","))), opt(&WS), python_literal(")"), opt(&WS), python_literal(":"), opt(&WS), &block), seq!(&with_item, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &with_item, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &with_item)))), opt(&WS), python_literal(":"), opt(seq!(opt(&WS), &TYPE_COMMENT)), opt(&WS), &block)))
    ))).into_rc_dyn();
    ...
    let star_etc = star_etc.set(tag("star_etc", choice!(
        seq!(python_literal("*"), opt(&WS), choice!(seq!(&param_no_default, opt(seq!(opt(&WS), &param_maybe_default, opt(seq!(opt(&WS), &param_maybe_default)))), opt(seq!(opt(&WS), &kwds))), seq!(&param_no_default_star_annotation, opt(seq!(opt(&WS), &param_maybe_default, opt(seq!(opt(&WS), &param_maybe_default)))), opt(seq!(opt(&WS), &kwds))), seq!(python_literal(","), opt(&WS), &param_maybe_default, opt(seq!(opt(&WS), &param_maybe_default)), opt(seq!(opt(&WS), &kwds))))),
        &kwds
    ))).into_rc_dyn();
    let slash_with_default = slash_with_default.set(tag("slash_with_default", seq!(opt(seq!(&param_no_default, opt(seq!(opt(&WS), &param_no_default)), opt(&WS))), &param_with_default, opt(seq!(opt(&WS), &param_with_default)), opt(&WS), python_literal("/"), opt(seq!(opt(&WS), python_literal(",")))))).into_rc_dyn();
    let slash_no_default = slash_no_default.set(tag("slash_no_default", seq!(&param_no_default, opt(seq!(opt(&WS), &param_no_default)), opt(&WS), python_literal("/"), opt(seq!(opt(&WS), python_literal(",")))))).into_rc_dyn();
    let parameters = parameters.set(tag("parameters", choice!(
        seq!(&slash_no_default, opt(seq!(opt(&WS), &param_no_default, opt(seq!(opt(&WS), &param_no_default)))), opt(seq!(opt(&WS), &param_with_default, opt(seq!(opt(&WS), &param_with_default)))), opt(seq!(opt(&WS), &star_etc))),
        seq!(&slash_with_default, opt(seq!(opt(&WS), &param_with_default, opt(seq!(opt(&WS), &param_with_default)))), opt(seq!(opt(&WS), &star_etc))),
        seq!(&param_no_default, opt(seq!(opt(&WS), &param_no_default)), opt(seq!(opt(&WS), &param_with_default, opt(seq!(opt(&WS), &param_with_default)))), opt(seq!(opt(&WS), &star_etc))),
        seq!(&param_with_default, opt(seq!(opt(&WS), &param_with_default)), opt(seq!(opt(&WS), &star_etc))),
        &star_etc
    ))).into_rc_dyn();
    ...
    let function_def = function_def.set(tag("function_def", choice!(
        seq!(python_literal("@"), opt(&WS), &named_expression, opt(&WS), &NEWLINE, opt(seq!(opt(&WS), python_literal("@"), opt(&WS), opt(seq!(&WS, opt(&WS))), opt(seq!(&WS, opt(seq!(opt(&WS), &WS)), opt(&WS))), &named_expression, opt(&WS), opt(seq!(&WS, opt(&WS))), opt(seq!(&WS, opt(seq!(opt(&WS), &WS)), opt(&WS))), &NEWLINE, opt(seq!(opt(&WS), python_literal("@"), opt(&WS), opt(seq!(&WS, opt(&WS))), opt(seq!(&WS, opt(seq!(opt(&WS), &WS)), opt(&WS))), &named_expression, opt(&WS), opt(seq!(&WS, opt(&WS))), opt(seq!(&WS, opt(seq!(opt(&WS), &WS)), opt(&WS))), &NEWLINE)))), opt(&WS), &function_def_raw),
        &function_def_raw
    ))).into_rc_dyn();
    ...
    let class_def = class_def.set(tag("class_def", choice!(
        seq!(python_literal("@"), opt(&WS), &named_expression, opt(&WS), &NEWLINE, opt(seq!(opt(&WS), python_literal("@"), opt(&WS), opt(seq!(&WS, opt(&WS))), &named_expression, opt(&WS), opt(seq!(&WS, opt(&WS))), &NEWLINE, opt(seq!(opt(&WS), python_literal("@"), opt(&WS), opt(seq!(&WS, opt(&WS))), &named_expression, opt(&WS), opt(seq!(&WS, opt(&WS))), &NEWLINE)))), opt(&WS), &class_def_raw),
        &class_def_raw
    ))).into_rc_dyn();
    let decorators = decorators.set(tag("decorators", seq!(python_literal("@"), opt(&WS), &named_expression, opt(&WS), &NEWLINE, opt(seq!(opt(&WS), python_literal("@"), opt(&WS), &named_expression, opt(&WS), &NEWLINE, opt(seq!(opt(&WS), python_literal("@"), opt(&WS), &named_expression, opt(&WS), &NEWLINE))))))).into_rc_dyn();
    let block = block.set(tag("block", choice!(
        seq!(&NEWLINE, opt(&WS), &INDENT, opt(&WS), &statements, opt(&WS), &DEDENT),
        seq!(&simple_stmt, opt(&WS), choice!(&NEWLINE, seq!(opt(seq!(python_literal(";"), opt(&WS), opt(seq!(&WS, opt(&WS))), &simple_stmt, opt(seq!(opt(&WS), python_literal(";"), opt(&WS), opt(seq!(&WS, opt(&WS))), &simple_stmt)), opt(&WS))), opt(seq!(python_literal(";"), opt(&WS))), &NEWLINE)))
    ))).into_rc_dyn();
    let dotted_name = dotted_name.set(tag("dotted_name", seq!(&NAME, opt(seq!(opt(&WS), python_literal("."), opt(&WS), &NAME, opt(seq!(opt(&WS), python_literal("."), opt(&WS), &NAME))))))).into_rc_dyn();
    ...
    let dotted_as_names = dotted_as_names.set(tag("dotted_as_names", seq!(&dotted_as_name, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &dotted_as_name, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &dotted_as_name))))))).into_rc_dyn();
    ...
    let import_from_as_names = import_from_as_names.set(tag("import_from_as_names", seq!(&import_from_as_name, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &import_from_as_name, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &import_from_as_name))))))).into_rc_dyn();
    ...
    let nonlocal_stmt = nonlocal_stmt.set(tag("nonlocal_stmt", seq!(python_literal("nonlocal"), opt(&WS), &NAME, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &NAME, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &NAME))))))).into_rc_dyn();
    let global_stmt = global_stmt.set(tag("global_stmt", seq!(python_literal("global"), opt(&WS), &NAME, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &NAME, opt(seq!(opt(&WS), python_literal(","), opt(&WS), &NAME))))))).into_rc_dyn();
    ...
    let assignment = assignment.set(tag("assignment", choice!(
        ...
        seq!(&star_targets, opt(&WS), python_literal("="), opt(seq!(opt(&WS), &star_targets, opt(&WS), python_literal("="), opt(seq!(opt(&WS), &star_targets, opt(&WS), python_literal("="))))), opt(&WS), choice!(&yield_expr, &star_expressions), opt(seq!(opt(&WS), &TYPE_COMMENT))),
    ...
    let simple_stmts = simple_stmts.set(tag("simple_stmts", seq!(&simple_stmt, opt(&WS), choice!(&NEWLINE, seq!(opt(seq!(python_literal(";"), opt(&WS), &simple_stmt, opt(seq!(opt(&WS), python_literal(";"), opt(&WS), &simple_stmt)), opt(&WS))), opt(seq!(python_literal(";"), opt(&WS))), &NEWLINE))))).into_rc_dyn();
    ...
    let statements = statements.set(tag("statements", seq!(&statement, opt(seq!(opt(&WS), &statement))))).into_rc_dyn();
    let func_type = func_type.set(tag("func_type", seq!(python_literal("("), opt(seq!(opt(&WS), &type_expressions)), opt(&WS), python_literal(")"), opt(&WS), python_literal("->"), opt(&WS), &expression, opt(seq!(opt(&WS), &NEWLINE, opt(seq!(opt(&WS), &NEWLINE)))), opt(&WS), &ENDMARKER))).into_rc_dyn();
    let eval = eval.set(tag("eval", seq!(&expressions, opt(seq!(opt(&WS), &NEWLINE, opt(seq!(opt(&WS), &NEWLINE)))), opt(&WS), &ENDMARKER))).into_rc_dyn();
