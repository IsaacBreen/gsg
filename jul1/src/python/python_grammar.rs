use std::rc::Rc;
use crate::{choice, seq, repeat0, repeat0 as repeat0, repeat1, opt, eat_char_choice, eat_string, eat_char_range, forward_ref, eps, python_newline, indent, dedent, DynCombinator, CombinatorTrait, symbol};
...
pub fn python_file() -> Rc<DynCombinator> {
    ...
    let expression_without_invalid = Rc::new(expression_without_invalid.set(choice!(
        seq!(&conjunction, opt(choice!(seq!(repeat0(choice!(seq!(eat_string("or"), &conjunction)))))), opt(choice!(seq!(eat_string("if"), &disjunction, eat_string("else"), &expression)))),
    ...
    let type_expressions = Rc::new(type_expressions.set(choice!(
        seq!(choice!(seq!(&disjunction, opt(choice!(seq!(eat_string("if"), &disjunction, eat_string("else"), &expression)))), seq!(&lambdef)), repeat0(eat_string(",")), opt(choice!(seq!(eat_string("*"), &expression, opt(choice!(seq!(eat_string(","), eat_string("**"), &expression)))), seq!(eat_string("**"), &expression)))),
        seq!(eat_string("*"), &expression, opt(choice!(seq!(eat_string(","), eat_string("**"), &expression)))),
        seq!(eat_string("**"), &expression)
    )).into_boxed());
    let del_t_atom = Rc::new(del_t_atom.set(choice!(
        seq!(&NAME),
        seq!(eat_string("("), choice!(seq!(&del_target, eat_string(")")), seq!(opt(choice!(seq!(&del_targets))), eat_string(")")))),
        seq!(eat_string("["), opt(choice!(seq!(&del_targets))), eat_string("]"))
    )).into_boxed());
    let del_target = Rc::new(del_target.set(choice!(
        seq!(choice!(seq!(&NAME), seq!(eat_string("True")), seq!(eat_string("False")), seq!(eat_string("None")), seq!(&strings), seq!(&NUMBER), seq!(&tuple), seq!(&group), seq!(&genexp), seq!(&list), seq!(&listcomp), seq!(&dict), seq!(&set), seq!(&dictcomp), seq!(&setcomp), seq!(eat_string("..."))), opt(choice!(seq!(repeat0(choice!(seq!(&strings), seq!(&tuple), seq!(&group), seq!(&genexp), seq!(&list), seq!(&listcomp), seq!(&dict), seq!(&set), seq!(&dictcomp), seq!(&setcomp)))))), opt(choice!(seq!(repeat0(choice!(seq!(eat_string("."), &NAME), seq!(eat_string("["), &slices, eat_string("]")), seq!(&genexp), seq!(eat_string("("), opt(choice!(seq!(&arguments))), eat_string(")"))))))), choice!(seq!(eat_string("."), &NAME), seq!(eat_string("["), &slices, eat_string("]")))),
        seq!(&del_t_atom)
    )).into_boxed());
    let del_targets = Rc::new(del_targets.set(choice!(
        seq!(&del_target, repeat0(eat_string(",")), opt(choice!(seq!(eat_string(",")))))
    )).into_boxed());
    ...
    let star_atom = Rc::new(star_atom.set(choice!(
        seq!(&NAME),
        seq!(eat_string("("), choice!(seq!(&target_with_star_atom, eat_string(")")), seq!(opt(choice!(seq!(&star_targets_tuple_seq))), eat_string(")")))),
    ...
    let star_targets_tuple_seq = Rc::new(star_targets_tuple_seq.set(choice!(
        seq!(&star_target, choice!(seq!(repeat0(choice!(seq!(eat_string(","), &star_target))), opt(choice!(seq!(eat_string(","))))), seq!(eat_string(","))))
    )).into_boxed());
    let star_targets_list_seq = Rc::new(star_targets_list_seq.set(choice!(
        seq!(&star_target, repeat0(eat_string(",")), opt(choice!(seq!(eat_string(",")))))
    )).into_boxed());
    let star_targets = Rc::new(star_targets.set(choice!(
        seq!(&star_target, opt(choice!(seq!(repeat0(choice!(seq!(eat_string(","), &star_target))), opt(choice!(seq!(eat_string(","))))))))
    )).into_boxed());
    ...
    let kwargs = Rc::new(kwargs.set(choice!(
        seq!(&kwarg_or_starred, repeat0(eat_string(",")), opt(choice!(seq!(&kwarg_or_double_starred, repeat0(eat_string(",")))))),
        seq!(&kwarg_or_double_starred, repeat0(eat_string(",")))
    )).into_boxed());
    let args = Rc::new(args.set(choice!(
        seq!(choice!(seq!(&starred_expression), seq!(&NAME, eat_string(":="), &expression), seq!(&disjunction, opt(choice!(seq!(eat_string("if"), &disjunction, eat_string("else"), &expression)))), seq!(&lambdef)), repeat0(eat_string(",")), opt(choice!(seq!(eat_string(","), &kwargs)))),
        seq!(&kwargs)
    )).into_boxed());
    ...
    let for_if_clause = Rc::new(for_if_clause.set(choice!(
        seq!(eat_string("async"), eat_string("for"), &star_targets, eat_string("in"), &disjunction, repeat0(choice!(seq!(eat_string("if"), &disjunction)))),
        seq!(eat_string("for"), &star_targets, eat_string("in"), &disjunction, repeat0(choice!(seq!(eat_string("if"), &disjunction))))
    )).into_boxed());
    let for_if_clauses = Rc::new(for_if_clauses.set(choice!(
        seq!(repeat0(&for_if_clause))
    )).into_boxed());
    let kvpair = Rc::new(kvpair.set(choice!(
        seq!(choice!(seq!(&disjunction, opt(choice!(seq!(eat_string("if"), &disjunction, eat_string("else"), &expression)))), seq!(&lambdef)), eat_string(":"), &expression)
    )).into_boxed());
    ...
    let double_starred_kvpairs = Rc::new(double_starred_kvpairs.set(choice!(
        seq!(&double_starred_kvpair, repeat0(eat_string(",")), opt(choice!(seq!(eat_string(",")))))
    )).into_boxed());
    ...
    let strings = Rc::new(strings.set(choice!(
        seq!(repeat0(choice!(seq!(&FSTRING_START, repeat0(&fstring_middle), &FSTRING_END), seq!(&STRING))))
    )).into_boxed());
    let string = Rc::new(string.set(choice!(
        seq!(&STRING)
    )).into_boxed());
    let fstring = Rc::new(fstring.set(choice!(
        seq!(&FSTRING_START, repeat0(&fstring_middle), &FSTRING_END)
    )).into_boxed());
    ...
    let fstring_full_format_spec = Rc::new(fstring_full_format_spec.set(choice!(
        seq!(eat_string(":"), repeat0(&fstring_format_spec))
    )).into_boxed());
    ...
    let lambda_param_maybe_default = Rc::new(lambda_param_maybe_default.set(choice!(
        seq!(&lambda_param, opt(choice!(seq!(&default))), opt(choice!(seq!(eat_string(",")))))
    )).into_boxed());
    let lambda_param_with_default = Rc::new(lambda_param_with_default.set(choice!(
        seq!(&lambda_param, &default, opt(choice!(seq!(eat_string(",")))))
    )).into_boxed());
    let lambda_param_no_default = Rc::new(lambda_param_no_default.set(choice!(
        seq!(&lambda_param, opt(choice!(seq!(eat_string(",")))))
    )).into_boxed());
    ...
    let lambda_star_etc = Rc::new(lambda_star_etc.set(choice!(
        seq!(eat_string("*"), choice!(seq!(&lambda_param_no_default, repeat0(&lambda_param_maybe_default), opt(choice!(seq!(&lambda_kwds)))), seq!(eat_string(","), repeat0(&lambda_param_maybe_default), opt(choice!(seq!(&lambda_kwds)))))),
        seq!(&lambda_kwds)
    )).into_boxed());
    let lambda_slash_with_default = Rc::new(lambda_slash_with_default.set(choice!(
        seq!(repeat0(&lambda_param_no_default), repeat0(&lambda_param_with_default), eat_string("/"), opt(choice!(seq!(eat_string(",")))))
    )).into_boxed());
    let lambda_slash_no_default = Rc::new(lambda_slash_no_default.set(choice!(
        seq!(repeat0(&lambda_param_no_default), eat_string("/"), opt(choice!(seq!(eat_string(",")))))
    )).into_boxed());
    let lambda_parameters = Rc::new(lambda_parameters.set(choice!(
        seq!(&lambda_slash_no_default, repeat0(&lambda_param_no_default), repeat0(&lambda_param_with_default), opt(choice!(seq!(&lambda_star_etc)))),
        seq!(&lambda_slash_with_default, repeat0(&lambda_param_with_default), opt(choice!(seq!(&lambda_star_etc)))),
        seq!(repeat0(&lambda_param_no_default), repeat0(&lambda_param_with_default), opt(choice!(seq!(&lambda_star_etc)))),
        seq!(repeat0(&lambda_param_with_default), opt(choice!(seq!(&lambda_star_etc)))),
        seq!(&lambda_star_etc)
    )).into_boxed());
    ...
    let atom = Rc::new(atom.set(choice!(
        seq!(choice!(seq!(&NAME), seq!(eat_string("True")), seq!(eat_string("False")), seq!(eat_string("None")), seq!(&strings), seq!(&NUMBER), seq!(&tuple), seq!(&group), seq!(&genexp), seq!(&list), seq!(&listcomp), seq!(&dict), seq!(&set), seq!(&dictcomp), seq!(&setcomp), seq!(eat_string("..."))), opt(choice!(seq!(repeat0(choice!(seq!(&strings), seq!(&tuple), seq!(&group), seq!(&genexp), seq!(&list), seq!(&listcomp), seq!(&dict), seq!(&set), seq!(&dictcomp), seq!(&setcomp)))))))
    )).into_boxed());
    let slice = Rc::new(slice.set(choice!(
        seq!(choice!(seq!(opt(choice!(seq!(&disjunction, opt(choice!(seq!(eat_string("if"), &disjunction, eat_string("else"), &expression)))), seq!(&lambdef))), eat_string(":"), opt(choice!(seq!(&expression))), opt(choice!(seq!(eat_string(":"), opt(choice!(seq!(&expression))))))), seq!(&NAME, eat_string(":="), &expression), seq!(&disjunction, opt(choice!(seq!(eat_string("if"), &disjunction, eat_string("else"), &expression)))), seq!(&lambdef)), opt(choice!(seq!(repeat0(choice!(seq!(eat_string(":"), opt(choice!(seq!(&expression))), opt(choice!(seq!(eat_string(":"), opt(choice!(seq!(&expression)))))))))))))
    )).into_boxed());
    let slices = Rc::new(slices.set(choice!(
        seq!(&slice),
        seq!(choice!(seq!(&slice), seq!(&starred_expression)), repeat0(eat_string(",")), opt(choice!(seq!(eat_string(",")))))
    )).into_boxed());
    let primary = Rc::new(primary.set(choice!(
        seq!(&atom, opt(choice!(seq!(repeat0(choice!(seq!(eat_string("."), &NAME), seq!(&genexp), seq!(eat_string("("), opt(choice!(seq!(&arguments))), eat_string(")")), seq!(eat_string("["), &slices, eat_string("]"))))))))
    )).into_boxed());
    ...
    let power = Rc::new(power.set(choice!(
        seq!(&await_primary, opt(choice!(seq!(eat_string("**"), &factor))))
    )).into_boxed());
    ...
    let term = Rc::new(term.set(choice!(
        seq!(&factor, opt(choice!(seq!(repeat0(choice!(seq!(eat_string("*"), &factor), seq!(eat_string("/"), &factor), seq!(eat_string("//"), &factor), seq!(eat_string("%"), &factor), seq!(eat_string("@"), &factor)))))))
    )).into_boxed());
    let sum = Rc::new(sum.set(choice!(
        seq!(&term, opt(choice!(seq!(repeat0(choice!(seq!(eat_string("+"), &term), seq!(eat_string("-"), &term)))))))
    )).into_boxed());
    let shift_expr = Rc::new(shift_expr.set(choice!(
        seq!(&sum, opt(choice!(seq!(repeat0(choice!(seq!(eat_string("<<"), &sum), seq!(eat_string(">>"), &sum)))))))
    )).into_boxed());
    let bitwise_and = Rc::new(bitwise_and.set(choice!(
        seq!(&shift_expr, opt(choice!(seq!(repeat0(choice!(seq!(eat_string("&"), &shift_expr)))))))
    )).into_boxed());
    let bitwise_xor = Rc::new(bitwise_xor.set(choice!(
        seq!(&bitwise_and, opt(choice!(seq!(repeat0(choice!(seq!(eat_string("^"), &bitwise_and)))))))
    )).into_boxed());
    let bitwise_or = Rc::new(bitwise_or.set(choice!(
        seq!(&bitwise_xor, opt(choice!(seq!(repeat0(choice!(seq!(eat_string("|"), &bitwise_xor)))))))
    )).into_boxed());
    ...
    let comparison = Rc::new(comparison.set(choice!(
        seq!(&bitwise_or, opt(choice!(seq!(repeat0(&compare_op_bitwise_or_pair)))))
    )).into_boxed());
    ...
    let conjunction = Rc::new(conjunction.set(choice!(
        seq!(&inversion, opt(choice!(seq!(repeat0(choice!(seq!(eat_string("and"), &inversion)))))))
    )).into_boxed());
    let disjunction = Rc::new(disjunction.set(choice!(
        seq!(&conjunction, opt(choice!(seq!(repeat0(choice!(seq!(eat_string("or"), &conjunction)))))))
    )).into_boxed());
    let named_expression = Rc::new(named_expression.set(choice!(
        seq!(&NAME, eat_string(":="), &expression),
        seq!(&disjunction, opt(choice!(seq!(eat_string("if"), &disjunction, eat_string("else"), &expression)))),
        seq!(&lambdef)
    )).into_boxed());
    ...
    let star_named_expressions = Rc::new(star_named_expressions.set(choice!(
        seq!(&star_named_expression, repeat0(eat_string(",")), opt(choice!(seq!(eat_string(",")))))
    )).into_boxed());
    ...
    let star_expressions = Rc::new(star_expressions.set(choice!(
        seq!(&star_expression, opt(choice!(seq!(repeat0(choice!(seq!(eat_string(","), &star_expression))), opt(choice!(seq!(eat_string(","))))), seq!(eat_string(",")))))
    )).into_boxed());
    let yield_expr = Rc::new(yield_expr.set(choice!(
        seq!(eat_string("yield"), opt(choice!(seq!(eat_string("from"), &expression), seq!(&star_expressions))))
    )).into_boxed());
    ...
    let expressions = Rc::new(expressions.set(choice!(
        seq!(&expression, opt(choice!(seq!(repeat0(choice!(seq!(eat_string(","), &expression))), opt(choice!(seq!(eat_string(","))))), seq!(eat_string(",")))))
    )).into_boxed());
    ...
    let type_param_seq = Rc::new(type_param_seq.set(choice!(
        seq!(&type_param, repeat0(eat_string(",")), opt(choice!(seq!(eat_string(",")))))
    )).into_boxed());
    ...
    let keyword_patterns = Rc::new(keyword_patterns.set(choice!(
        seq!(&keyword_pattern, repeat0(eat_string(",")))
    )).into_boxed());
    let positional_patterns = Rc::new(positional_patterns.set(choice!(
        seq!(choice!(seq!(&as_pattern), seq!(&or_pattern)), repeat0(eat_string(",")))
    )).into_boxed());
    let class_pattern = Rc::new(class_pattern.set(choice!(
        seq!(&NAME, opt(choice!(seq!(repeat0(choice!(seq!(eat_string("."), &NAME)))))), eat_string("("), choice!(seq!(eat_string(")")), seq!(&positional_patterns, choice!(seq!(opt(choice!(seq!(eat_string(",")))), eat_string(")")), seq!(eat_string(","), &keyword_patterns, opt(choice!(seq!(eat_string(",")))), eat_string(")")))), seq!(&keyword_patterns, opt(choice!(seq!(eat_string(",")))), eat_string(")"))))
    )).into_boxed());
    ...
    let items_pattern = Rc::new(items_pattern.set(choice!(
        seq!(&key_value_pattern, repeat0(eat_string(",")))
    )).into_boxed());
    let mapping_pattern = Rc::new(mapping_pattern.set(choice!(
        seq!(eat_string("{"), choice!(seq!(eat_string("}")), seq!(&double_star_pattern, opt(choice!(seq!(eat_string(",")))), eat_string("}")), seq!(&items_pattern, choice!(seq!(eat_string(","), &double_star_pattern, opt(choice!(seq!(eat_string(",")))), eat_string("}")), seq!(opt(choice!(seq!(eat_string(",")))), eat_string("}"))))))
    )).into_boxed());
    ...
    let maybe_sequence_pattern = Rc::new(maybe_sequence_pattern.set(choice!(
        seq!(&maybe_star_pattern, repeat0(eat_string(",")), opt(choice!(seq!(eat_string(",")))))
    )).into_boxed());
    ...
    let name_or_attr = Rc::new(name_or_attr.set(choice!(
        seq!(&NAME, opt(choice!(seq!(repeat0(choice!(seq!(eat_string("."), &NAME)))))))
    )).into_boxed());
    ...
    let or_pattern = Rc::new(or_pattern.set(choice!(
        seq!(&closed_pattern, repeat0(eat_string("|")))
    )).into_boxed());
    ...
    let match_stmt = Rc::new(match_stmt.set(choice!(
        seq!(eat_string("atc"), &subject_expr, eat_string(":"), &NEWLINE, &INDENT, repeat0(&case_block), &DEDENT)
    )).into_boxed());
    ...
    let except_block = Rc::new(except_block.set(choice!(
        seq!(eat_string("except"), choice!(seq!(&expression, opt(choice!(seq!(eat_string("as"), &NAME))), eat_string(":"), &block), seq!(eat_string(":"), &block)))
    )).into_boxed());
    let try_stmt = Rc::new(try_stmt.set(choice!(
        seq!(eat_string("try"), eat_string(":"), &block, choice!(seq!(&finally_block), seq!(repeat0(&except_block), opt(choice!(seq!(&else_block))), opt(choice!(seq!(&finally_block)))), seq!(repeat0(&except_star_block), opt(choice!(seq!(&else_block))), opt(choice!(seq!(&finally_block))))))
    )).into_boxed());
    let with_item = Rc::new(with_item.set(choice!(
        seq!(&expression, opt(choice!(seq!(eat_string("as"), &star_target))))
    )).into_boxed());
    let with_stmt = Rc::new(with_stmt.set(choice!(
        seq!(eat_string("with"), choice!(seq!(eat_string("("), &with_item, repeat0(eat_string(",")), opt(choice!(seq!(eat_string(",")))), eat_string(")"), eat_string(":"), opt(choice!(seq!(&TYPE_COMMENT))), &block), seq!(&with_item, repeat0(eat_string(",")), eat_string(":"), opt(choice!(seq!(&TYPE_COMMENT))), &block))),
        seq!(eat_string("async"), eat_string("with"), choice!(seq!(eat_string("("), &with_item, repeat0(eat_string(",")), opt(choice!(seq!(eat_string(",")))), eat_string(")"), eat_string(":"), &block), seq!(&with_item, repeat0(eat_string(",")), eat_string(":"), opt(choice!(seq!(&TYPE_COMMENT))), &block)))
    )).into_boxed());
    ...
    let elif_stmt = Rc::new(elif_stmt.set(choice!(
        seq!(eat_string("elif"), &named_expression, eat_string(":"), &block, opt(choice!(seq!(&elif_stmt), seq!(&else_block))))
    )).into_boxed());
    let if_stmt = Rc::new(if_stmt.set(choice!(
        seq!(eat_string("if"), &named_expression, eat_string(":"), &block, opt(choice!(seq!(&elif_stmt), seq!(&else_block))))
    )).into_boxed());
    ...
    let param_maybe_default = Rc::new(param_maybe_default.set(choice!(
        seq!(&param, opt(choice!(seq!(&default))), opt(choice!(seq!(eat_string(","), opt(choice!(seq!(&TYPE_COMMENT)))), seq!(&TYPE_COMMENT))))
    )).into_boxed());
    let param_with_default = Rc::new(param_with_default.set(choice!(
        seq!(&param, &default, opt(choice!(seq!(eat_string(","), opt(choice!(seq!(&TYPE_COMMENT)))), seq!(&TYPE_COMMENT))))
    )).into_boxed());
    let param_no_default_star_annotation = Rc::new(param_no_default_star_annotation.set(choice!(
        seq!(&param_star_annotation, opt(choice!(seq!(eat_string(","), opt(choice!(seq!(&TYPE_COMMENT)))), seq!(&TYPE_COMMENT))))
    )).into_boxed());
    let param_no_default = Rc::new(param_no_default.set(choice!(
        seq!(&param, opt(choice!(seq!(eat_string(","), opt(choice!(seq!(&TYPE_COMMENT)))), seq!(&TYPE_COMMENT))))
    )).into_boxed());
    let kwds = Rc::new(kwds.set(choice!(
        seq!(eat_string("**"), &param_no_default)
    )).into_boxed());
    let star_etc = Rc::new(star_etc.set(choice!(
        seq!(eat_string("*"), choice!(seq!(&param_no_default, repeat0(&param_maybe_default), opt(choice!(seq!(&kwds)))), seq!(&param_no_default_star_annotation, repeat0(&param_maybe_default), opt(choice!(seq!(&kwds)))), seq!(eat_string(","), repeat0(&param_maybe_default), opt(choice!(seq!(&kwds)))))),
        seq!(&kwds)
    )).into_boxed());
    let slash_with_default = Rc::new(slash_with_default.set(choice!(
        seq!(repeat0(&param_no_default), repeat0(&param_with_default), eat_string("/"), opt(choice!(seq!(eat_string(",")))))
    )).into_boxed());
    let slash_no_default = Rc::new(slash_no_default.set(choice!(
        seq!(repeat0(&param_no_default), eat_string("/"), opt(choice!(seq!(eat_string(",")))))
    )).into_boxed());
    let parameters = Rc::new(parameters.set(choice!(
        seq!(&slash_no_default, repeat0(&param_no_default), repeat0(&param_with_default), opt(choice!(seq!(&star_etc)))),
        seq!(&slash_with_default, repeat0(&param_with_default), opt(choice!(seq!(&star_etc)))),
        seq!(repeat0(&param_no_default), repeat0(&param_with_default), opt(choice!(seq!(&star_etc)))),
        seq!(repeat0(&param_with_default), opt(choice!(seq!(&star_etc)))),
        seq!(&star_etc)
    )).into_boxed());
    ...
    let function_def = Rc::new(function_def.set(choice!(
        seq!(eat_string("@"), &named_expression, &NEWLINE, opt(choice!(seq!(repeat0(choice!(seq!(eat_string("@"), &named_expression, &NEWLINE)))))), &function_def_raw),
    ...
    let class_def = Rc::new(class_def.set(choice!(
        seq!(eat_string("@"), &named_expression, &NEWLINE, opt(choice!(seq!(repeat0(choice!(seq!(eat_string("@"), &named_expression, &NEWLINE)))))), &class_def_raw),
        seq!(&class_def_raw)
    )).into_boxed());
    let decorators = Rc::new(decorators.set(choice!(
        seq!(eat_string("@"), &named_expression, &NEWLINE, opt(choice!(seq!(repeat0(choice!(seq!(eat_string("@"), &named_expression, &NEWLINE)))))))
    )).into_boxed());
    let block = Rc::new(block.set(choice!(
        seq!(&NEWLINE, &INDENT, &statements, &DEDENT),
        seq!(&simple_stmt, choice!(seq!(&NEWLINE), seq!(repeat0(eat_string(";")), opt(choice!(seq!(eat_string(";")))), &NEWLINE)))
    )).into_boxed());
    ...
    let dotted_as_names = Rc::new(dotted_as_names.set(choice!(
        seq!(&dotted_as_name, repeat0(eat_string(",")))
    )).into_boxed());
    ...
    let import_from_as_names = Rc::new(import_from_as_names.set(choice!(
        seq!(&import_from_as_name, repeat0(eat_string(",")))
    )).into_boxed());
    ...
    let import_from = Rc::new(import_from.set(choice!(
        seq!(eat_string("from"), repeat0(choice!(seq!(eat_string(".")), seq!(eat_string("...")))), choice!(seq!(&dotted_name, eat_string("import"), &import_from_targets), seq!(eat_string("import"), &import_from_targets)))
    )).into_boxed());
    ...
    let nonlocal_stmt = Rc::new(nonlocal_stmt.set(choice!(
        seq!(eat_string("nonlocal"), &NAME, repeat0(eat_string(",")))
    )).into_boxed());
    let global_stmt = Rc::new(global_stmt.set(choice!(
        seq!(eat_string("global"), &NAME, repeat0(eat_string(",")))
    )).into_boxed());
    let raise_stmt = Rc::new(raise_stmt.set(choice!(
        seq!(eat_string("raise"), opt(choice!(seq!(&expression, opt(choice!(seq!(eat_string("from"), &expression)))))))
    )).into_boxed());
    ...
    let assignment = Rc::new(assignment.set(choice!(
        ...
        seq!(&star_targets, eat_string("="), opt(choice!(seq!(repeat0(choice!(seq!(&star_targets, eat_string("="))))))), choice!(seq!(&yield_expr), seq!(&star_expressions)), opt(choice!(seq!(&TYPE_COMMENT)))),
        seq!(&single_target, &augassign, choice!(seq!(&yield_expr), seq!(&star_expressions)))
    )).into_boxed());
    let compound_stmt = Rc::new(compound_stmt.set(choice!(
        seq!(choice!(seq!(&function_def), seq!(&if_stmt), seq!(&class_def), seq!(&with_stmt), seq!(&for_stmt), seq!(&try_stmt), seq!(&while_stmt), seq!(&match_stmt)), opt(choice!(seq!(repeat0(choice!(seq!(&function_def), seq!(&if_stmt), seq!(&class_def), seq!(&with_stmt), seq!(&for_stmt), seq!(&try_stmt), seq!(&while_stmt)))))))
    )).into_boxed());
    let simple_stmt = Rc::new(simple_stmt.set(choice!(
        seq!(choice!(seq!(&assignment), seq!(&type_alias), seq!(&star_expressions), seq!(&return_stmt), seq!(&import_stmt), seq!(&raise_stmt), seq!(eat_string("pass")), seq!(&del_stmt), seq!(&yield_stmt), seq!(&assert_stmt), seq!(eat_string("break")), seq!(eat_string("continue")), seq!(&global_stmt), seq!(&nonlocal_stmt)), opt(choice!(seq!(repeat0(choice!(seq!(&type_alias), seq!(&return_stmt), seq!(&import_stmt), seq!(&raise_stmt), seq!(&del_stmt), seq!(&yield_stmt), seq!(&assert_stmt), seq!(&global_stmt), seq!(&nonlocal_stmt)))))))
    )).into_boxed());
    ...
    let statements = Rc::new(statements.set(choice!(
        seq!(repeat0(&statement))
    )).into_boxed());
    let func_type = Rc::new(func_type.set(choice!(
        seq!(eat_string("("), opt(choice!(seq!(&type_expressions))), eat_string(")"), eat_string("->"), &expression, repeat0(&NEWLINE), &ENDMARKER)
    )).into_boxed());
    let eval = Rc::new(eval.set(choice!(
        seq!(&expressions, repeat0(&NEWLINE), &ENDMARKER)
    )).into_boxed());
    let interactive = Rc::new(interactive.set(choice!(
        seq!(&statement_newline)
    )).into_boxed());
    let file = Rc::new(file.set(choice!(
        seq!(opt(choice!(seq!(&statements))), repeat0(&ENDMARKER))
    )).into_boxed());
