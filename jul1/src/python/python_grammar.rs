use std::rc::Rc;
use crate::{cache_context, cached, symbol, Symbol, mutate_right_data, RightData, Choice, deferred, Combinator, CombinatorTrait, eat_char_choice, eat_char_range, eat_string, eps, Eps, forbid_follows, forbid_follows_check_not, forbid_follows_clear, Repeat1, Seq, tag, lookahead, negative_lookahead};
use crate::seq;
use crate::{opt_greedy as opt, choice_greedy as choice, seprep0_greedy as seprep0, seprep1_greedy as seprep1, repeat0_greedy as repeat0, repeat1_greedy as repeat1};

enum Forbidden {
    WS,
    NAME,
    TYPE_COMMENT,
    FSTRING_START,
    FSTRING_MIDDLE,
    FSTRING_END,
    NUMBER,
    STRING,
    NEWLINE,
    INDENT,
    DEDENT,
    ENDMARKER,
}

use super::python_tokenizer as token;

pub fn python_literal(s: &str) -> Combinator {
    let increment_scope_count = |right_data: &mut RightData| { Rc::make_mut(&mut right_data.right_data_inner).fields1.scope_count += 1; true };
    let decrement_scope_count = |right_data: &mut RightData| { Rc::make_mut(&mut right_data.right_data_inner).fields1.scope_count -= 1; true };

    match s {
        "(" | "[" | "{" => seq!(eat_string(s), mutate_right_data(increment_scope_count), forbid_follows_clear(), opt(&WS)),
        ")" | "]" | "}" => seq!(eat_string(s), mutate_right_data(decrement_scope_count), forbid_follows_clear(), opt(&WS)),
        _ => seq!(eat_string(s), forbid_follows_clear(), opt(&WS)),
    }
}
pub fn WS() -> Combinator { cached(tag("WS", crate::profile("WS", seq!(forbid_follows_check_not(Forbidden::WS as usize), token::WS().compile(), forbid_follows(&[Forbidden::INDENT as usize, Forbidden::DEDENT as usize]))))).into() }
pub fn NAME() -> Combinator { cached(seq!(tag("NAME", crate::profile("NAME", seq!(forbid_follows_check_not(Forbidden::NAME as usize), token::NAME().compile(), forbid_follows(&[Forbidden::NAME as usize, Forbidden::NUMBER as usize])))), opt(&WS))).into() }
pub fn TYPE_COMMENT() -> Combinator { cached(seq!(tag("TYPE_COMMENT", crate::profile("TYPE_COMMENT", seq!(token::TYPE_COMMENT().compile(), forbid_follows_clear()))), opt(&WS))).into() }
pub fn FSTRING_START() -> Combinator { cached(tag("FSTRING_START", crate::profile("FSTRING_START", seq!(token::FSTRING_START().compile(), forbid_follows(&[Forbidden::WS as usize, Forbidden::NEWLINE as usize]))))).into() }
pub fn FSTRING_MIDDLE() -> Combinator { cached(tag("FSTRING_MIDDLE", crate::profile("FSTRING_MIDDLE", seq!(forbid_follows_check_not(Forbidden::FSTRING_MIDDLE as usize), token::FSTRING_MIDDLE().compile(), forbid_follows(&[Forbidden::FSTRING_MIDDLE as usize, Forbidden::WS as usize]))))).into() }
pub fn FSTRING_END() -> Combinator { cached(seq!(tag("FSTRING_END", crate::profile("FSTRING_END", seq!(token::FSTRING_END().compile(), forbid_follows_clear()))), opt(&WS))).into() }
pub fn NUMBER() -> Combinator { cached(seq!(tag("NUMBER", crate::profile("NUMBER", seq!(forbid_follows_check_not(Forbidden::NUMBER as usize), token::NUMBER().compile(), forbid_follows(&[Forbidden::NUMBER as usize])))), opt(&WS))).into() }
pub fn STRING() -> Combinator { cached(seq!(tag("STRING", crate::profile("STRING", seq!(token::STRING().compile(), forbid_follows_clear()))), opt(&WS))).into() }
pub fn NEWLINE() -> Combinator { cached(tag("NEWLINE", crate::profile("NEWLINE", seq!(forbid_follows_check_not(Forbidden::NEWLINE as usize), token::NEWLINE().compile(), forbid_follows(&[Forbidden::WS as usize]))))).into() }
pub fn INDENT() -> Combinator { cached(tag("INDENT", crate::profile("INDENT", seq!(forbid_follows_check_not(Forbidden::INDENT as usize), token::INDENT().compile(), forbid_follows(&[Forbidden::WS as usize]))))).into() }
pub fn DEDENT() -> Combinator { cached(tag("DEDENT", crate::profile("DEDENT", seq!(forbid_follows_check_not(Forbidden::DEDENT as usize), token::DEDENT().compile(), forbid_follows(&[Forbidden::WS as usize]))))).into() }
pub fn ENDMARKER() -> Combinator { cached(seq!(tag("ENDMARKER", crate::profile("ENDMARKER", seq!(token::ENDMARKER().compile(), forbid_follows_clear()))), opt(&WS))).into() }

pub fn expression_without_invalid() -> Combinator {
    tag("expression_without_invalid", choice!(
        seq!(deferred(conjunction), repeat0(seq!(python_literal("or"), deferred(conjunction))), opt(seq!(python_literal("if"), deferred(conjunction), repeat0(seq!(python_literal("or"), deferred(conjunction))), python_literal("else"), choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef))))),
        seq!(python_literal("lambda"), opt(deferred(lambda_params)), python_literal(":"), choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef)))
    )).into()
}

pub fn func_type_comment() -> Combinator {
    tag("func_type_comment", choice!(
        seq!(deferred(NEWLINE), deferred(TYPE_COMMENT), lookahead(seq!(deferred(NEWLINE), deferred(INDENT)))),
        deferred(TYPE_COMMENT)
    )).into()
}

pub fn type_expressions() -> Combinator {
    tag("type_expressions", choice!(
        seq!(choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef)), repeat0(seq!(python_literal(","), deferred(expression))), opt(seq!(python_literal(","), choice!(seq!(python_literal("*"), choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef)), opt(seq!(python_literal(","), python_literal("**"), choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef))))), seq!(python_literal("**"), choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef))))))),
        seq!(python_literal("*"), choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef)), opt(seq!(python_literal(","), python_literal("**"), choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef))))),
        seq!(python_literal("**"), choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef)))
    )).into()
}

pub fn del_t_atom() -> Combinator {
    tag("del_t_atom", choice!(
        deferred(NAME),
        seq!(python_literal("("), choice!(seq!(choice!(seq!(choice!(deferred(NAME), python_literal("True"), python_literal("False"), python_literal("None"), seq!(lookahead(choice!(deferred(STRING), deferred(FSTRING_START))), deferred(strings)), deferred(NUMBER), seq!(lookahead(python_literal("(")), choice!(deferred(tuple), deferred(group), deferred(genexp))), seq!(lookahead(python_literal("[")), choice!(deferred(list), deferred(listcomp))), seq!(lookahead(python_literal("{")), choice!(deferred(dict), deferred(set), deferred(dictcomp), deferred(setcomp))), python_literal("...")), lookahead(deferred(t_lookahead)), repeat0(choice!(seq!(python_literal("."), deferred(NAME), lookahead(deferred(t_lookahead))), seq!(python_literal("["), choice!(seq!(deferred(slice), negative_lookahead(python_literal(","))), seq!(seprep1(choice!(deferred(slice), deferred(starred_expression)), python_literal(",")), opt(python_literal(",")))), python_literal("]"), lookahead(deferred(t_lookahead))), seq!(python_literal("("), choice!(seq!(choice!(seq!(deferred(NAME), python_literal(":="), choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef))), seq!(choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef)), negative_lookahead(python_literal(":=")))), repeat1(deferred(for_if_clause)), python_literal(")"), lookahead(deferred(t_lookahead))), seq!(opt(seq!(deferred(args), opt(python_literal(",")), lookahead(python_literal(")")))), python_literal(")"), lookahead(deferred(t_lookahead))))))), choice!(seq!(python_literal("."), deferred(NAME), negative_lookahead(deferred(t_lookahead))), seq!(python_literal("["), choice!(seq!(deferred(slice), negative_lookahead(python_literal(","))), seq!(seprep1(choice!(deferred(slice), deferred(starred_expression)), python_literal(",")), opt(python_literal(",")))), python_literal("]"), negative_lookahead(deferred(t_lookahead))))), deferred(del_t_atom)), python_literal(")")), seq!(opt(seq!(seprep1(deferred(del_target), python_literal(",")), opt(python_literal(",")))), python_literal(")")))),
        seq!(python_literal("["), opt(seq!(seprep1(deferred(del_target), python_literal(",")), opt(python_literal(",")))), python_literal("]"))
    )).into()
}

pub fn del_target() -> Combinator {
    cached(tag("del_target", choice!(
        seq!(choice!(deferred(NAME), python_literal("True"), python_literal("False"), python_literal("None"), seq!(lookahead(choice!(deferred(STRING), deferred(FSTRING_START))), deferred(strings)), deferred(NUMBER), seq!(lookahead(python_literal("(")), choice!(deferred(tuple), deferred(group), deferred(genexp))), seq!(lookahead(python_literal("[")), choice!(deferred(list), deferred(listcomp))), seq!(lookahead(python_literal("{")), choice!(deferred(dict), deferred(set), deferred(dictcomp), deferred(setcomp))), python_literal("...")), lookahead(deferred(t_lookahead)), repeat0(choice!(seq!(python_literal("."), deferred(NAME), lookahead(deferred(t_lookahead))), seq!(python_literal("["), choice!(seq!(deferred(slice), negative_lookahead(python_literal(","))), seq!(seprep1(choice!(deferred(slice), deferred(starred_expression)), python_literal(",")), opt(python_literal(",")))), python_literal("]"), lookahead(deferred(t_lookahead))), seq!(python_literal("("), choice!(seq!(choice!(seq!(deferred(NAME), python_literal(":="), choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef))), seq!(choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef)), negative_lookahead(python_literal(":=")))), repeat1(deferred(for_if_clause)), python_literal(")"), lookahead(deferred(t_lookahead))), seq!(opt(seq!(deferred(args), opt(python_literal(",")), lookahead(python_literal(")")))), python_literal(")"), lookahead(deferred(t_lookahead))))))), choice!(seq!(python_literal("."), deferred(NAME), negative_lookahead(deferred(t_lookahead))), seq!(python_literal("["), choice!(seq!(deferred(slice), negative_lookahead(python_literal(","))), seq!(seprep1(choice!(deferred(slice), deferred(starred_expression)), python_literal(",")), opt(python_literal(",")))), python_literal("]"), negative_lookahead(deferred(t_lookahead))))),
        deferred(del_t_atom)
    ))).into()
}

pub fn del_targets() -> Combinator {
    tag("del_targets", seq!(seprep1(deferred(del_target), python_literal(",")), opt(python_literal(",")))).into()
}

pub fn t_lookahead() -> Combinator {
    tag("t_lookahead", choice!(
        python_literal("("),
        python_literal("["),
        python_literal(".")
    )).into()
}

pub fn t_primary() -> Combinator {
    tag("t_primary", seq!(choice!(deferred(NAME), python_literal("True"), python_literal("False"), python_literal("None"), seq!(lookahead(choice!(deferred(STRING), deferred(FSTRING_START))), deferred(strings)), deferred(NUMBER), seq!(lookahead(python_literal("(")), choice!(deferred(tuple), deferred(group), deferred(genexp))), seq!(lookahead(python_literal("[")), choice!(deferred(list), deferred(listcomp))), seq!(lookahead(python_literal("{")), choice!(deferred(dict), deferred(set), deferred(dictcomp), deferred(setcomp))), python_literal("...")), lookahead(deferred(t_lookahead)), repeat0(choice!(seq!(python_literal("."), deferred(NAME), lookahead(deferred(t_lookahead))), seq!(python_literal("["), choice!(seq!(deferred(slice), negative_lookahead(python_literal(","))), seq!(seprep1(choice!(deferred(slice), deferred(starred_expression)), python_literal(",")), opt(python_literal(",")))), python_literal("]"), lookahead(deferred(t_lookahead))), seq!(python_literal("("), choice!(seq!(choice!(seq!(deferred(NAME), python_literal(":="), choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef))), seq!(choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef)), negative_lookahead(python_literal(":=")))), repeat1(deferred(for_if_clause)), python_literal(")"), lookahead(deferred(t_lookahead))), seq!(opt(seq!(deferred(args), opt(python_literal(",")), lookahead(python_literal(")")))), python_literal(")"), lookahead(deferred(t_lookahead))))))))).into()
}

pub fn single_subscript_attribute_target() -> Combinator {
    tag("single_subscript_attribute_target", seq!(deferred(t_primary), choice!(seq!(python_literal("."), deferred(NAME), negative_lookahead(deferred(t_lookahead))), seq!(python_literal("["), choice!(seq!(deferred(slice), negative_lookahead(python_literal(","))), seq!(seprep1(choice!(deferred(slice), deferred(starred_expression)), python_literal(",")), opt(python_literal(",")))), python_literal("]"), negative_lookahead(deferred(t_lookahead)))))).into()
}

pub fn single_target() -> Combinator {
    tag("single_target", choice!(
        deferred(single_subscript_attribute_target),
        deferred(NAME),
        seq!(python_literal("("), deferred(single_target), python_literal(")"))
    )).into()
}

pub fn star_atom() -> Combinator {
    tag("star_atom", choice!(
        deferred(NAME),
        seq!(python_literal("("), choice!(seq!(choice!(seq!(deferred(t_primary), choice!(seq!(python_literal("."), deferred(NAME), negative_lookahead(deferred(t_lookahead))), seq!(python_literal("["), choice!(seq!(deferred(slice), negative_lookahead(python_literal(","))), seq!(seprep1(choice!(deferred(slice), deferred(starred_expression)), python_literal(",")), opt(python_literal(",")))), python_literal("]"), negative_lookahead(deferred(t_lookahead))))), deferred(star_atom)), python_literal(")")), seq!(opt(seq!(deferred(star_target), choice!(seq!(repeat1(seq!(python_literal(","), deferred(star_target))), opt(python_literal(","))), python_literal(",")))), python_literal(")")))),
        seq!(python_literal("["), opt(seq!(seprep1(deferred(star_target), python_literal(",")), opt(python_literal(",")))), python_literal("]"))
    )).into()
}

pub fn target_with_star_atom() -> Combinator {
    cached(tag("target_with_star_atom", choice!(
        seq!(deferred(t_primary), choice!(seq!(python_literal("."), deferred(NAME), negative_lookahead(deferred(t_lookahead))), seq!(python_literal("["), choice!(seq!(deferred(slice), negative_lookahead(python_literal(","))), seq!(seprep1(choice!(deferred(slice), deferred(starred_expression)), python_literal(",")), opt(python_literal(",")))), python_literal("]"), negative_lookahead(deferred(t_lookahead))))),
        deferred(star_atom)
    ))).into()
}

pub fn star_target() -> Combinator {
    cached(tag("star_target", choice!(
        seq!(python_literal("*"), negative_lookahead(python_literal("*")), deferred(star_target)),
        deferred(target_with_star_atom)
    ))).into()
}

pub fn star_targets_tuple_seq() -> Combinator {
    tag("star_targets_tuple_seq", seq!(deferred(star_target), choice!(seq!(repeat1(seq!(python_literal(","), deferred(star_target))), opt(python_literal(","))), python_literal(",")))).into()
}

pub fn star_targets_list_seq() -> Combinator {
    tag("star_targets_list_seq", seq!(seprep1(deferred(star_target), python_literal(",")), opt(python_literal(",")))).into()
}

pub fn star_targets() -> Combinator {
    tag("star_targets", seq!(deferred(star_target), choice!(negative_lookahead(python_literal(",")), seq!(repeat0(seq!(python_literal(","), deferred(star_target))), opt(python_literal(",")))))).into()
}

pub fn kwarg_or_double_starred() -> Combinator {
    tag("kwarg_or_double_starred", choice!(
        seq!(deferred(NAME), python_literal("="), choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef))),
        seq!(python_literal("**"), choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef)))
    )).into()
}

pub fn kwarg_or_starred() -> Combinator {
    tag("kwarg_or_starred", choice!(
        seq!(deferred(NAME), python_literal("="), choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef))),
        seq!(python_literal("*"), choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef)))
    )).into()
}

pub fn starred_expression() -> Combinator {
    tag("starred_expression", seq!(python_literal("*"), choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef)))).into()
}

pub fn kwargs() -> Combinator {
    tag("kwargs", choice!(
        seq!(seprep1(deferred(kwarg_or_starred), python_literal(",")), opt(seq!(python_literal(","), seprep1(deferred(kwarg_or_double_starred), python_literal(","))))),
        seprep1(deferred(kwarg_or_double_starred), python_literal(","))
    )).into()
}

pub fn args() -> Combinator {
    tag("args", choice!(
        seq!(seprep1(choice!(deferred(starred_expression), seq!(choice!(seq!(deferred(NAME), python_literal(":="), choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef))), seq!(choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef)), negative_lookahead(python_literal(":=")))), negative_lookahead(python_literal("=")))), python_literal(",")), opt(seq!(python_literal(","), deferred(kwargs)))),
        deferred(kwargs)
    )).into()
}

pub fn arguments() -> Combinator {
    cached(tag("arguments", seq!(deferred(args), opt(python_literal(",")), lookahead(python_literal(")"))))).into()
}

pub fn dictcomp() -> Combinator {
    tag("dictcomp", seq!(
        python_literal("{"),
         choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef)),
         python_literal(":"),
         choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef)),
         repeat1(deferred(for_if_clause)),
         python_literal("}")
    )).into()
}

pub fn genexp() -> Combinator {
    tag("genexp", seq!(python_literal("("), choice!(seq!(deferred(NAME), python_literal(":="), choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef))), seq!(choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef)), negative_lookahead(python_literal(":=")))), repeat1(deferred(for_if_clause)), python_literal(")"))).into()
}

pub fn setcomp() -> Combinator {
    tag("setcomp", seq!(python_literal("{"), choice!(seq!(deferred(NAME), python_literal(":="), choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef))), seq!(choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef)), negative_lookahead(python_literal(":=")))), repeat1(deferred(for_if_clause)), python_literal("}"))).into()
}

pub fn listcomp() -> Combinator {
    tag("listcomp", seq!(python_literal("["), choice!(seq!(deferred(NAME), python_literal(":="), choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef))), seq!(choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef)), negative_lookahead(python_literal(":=")))), repeat1(deferred(for_if_clause)), python_literal("]"))).into()
}

pub fn for_if_clause() -> Combinator {
    tag("for_if_clause", choice!(
        seq!(python_literal("async"), python_literal("for"), deferred(star_targets), python_literal("in"), deferred(conjunction), repeat0(seq!(python_literal("or"), deferred(conjunction))), repeat0(seq!(python_literal("if"), deferred(conjunction), repeat0(seq!(python_literal("or"), deferred(conjunction)))))),
        seq!(python_literal("for"), deferred(star_targets), python_literal("in"), deferred(conjunction), repeat0(seq!(python_literal("or"), deferred(conjunction))), repeat0(seq!(python_literal("if"), deferred(conjunction), repeat0(seq!(python_literal("or"), deferred(conjunction))))))
    )).into()
}

pub fn for_if_clauses() -> Combinator {
    tag("for_if_clauses", repeat1(deferred(for_if_clause))).into()
}

pub fn kvpair() -> Combinator {
    tag("kvpair", seq!(choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef)), python_literal(":"), choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef)))).into()
}

pub fn double_starred_kvpair() -> Combinator {
    tag("double_starred_kvpair", choice!(
        seq!(python_literal("**"), deferred(bitwise_xor), repeat0(seq!(python_literal("|"), deferred(bitwise_xor)))),
        deferred(kvpair)
    )).into()
}

pub fn double_starred_kvpairs() -> Combinator {
    tag("double_starred_kvpairs", seq!(seprep1(deferred(double_starred_kvpair), python_literal(",")), opt(python_literal(",")))).into()
}

pub fn dict() -> Combinator {
    tag("dict", seq!(python_literal("{"), opt(deferred(double_starred_kvpairs)), python_literal("}"))).into()
}

pub fn set() -> Combinator {
    tag("set", seq!(python_literal("{"), seprep1(deferred(star_named_expression), python_literal(",")), opt(python_literal(",")), python_literal("}"))).into()
}

pub fn tuple() -> Combinator {
    tag("tuple", seq!(python_literal("("), opt(seq!(choice!(seq!(python_literal("*"), deferred(bitwise_or)), deferred(named_expression)), python_literal(","), opt(seq!(seprep1(deferred(star_named_expression), python_literal(",")), opt(python_literal(",")))))), python_literal(")"))).into()
}

pub fn list() -> Combinator {
    tag("list", seq!(python_literal("["), opt(seq!(seprep1(deferred(star_named_expression), python_literal(",")), opt(python_literal(",")))), python_literal("]"))).into()
}

pub fn strings() -> Combinator {
    cached(tag("strings", repeat1(choice!(seq!(deferred(FSTRING_START), opt(seq!(choice!(deferred(fstring_replacement_field), deferred(FSTRING_MIDDLE)), repeat0(deferred(fstring_middle)))), deferred(FSTRING_END)), deferred(STRING))))).into()
}

pub fn string() -> Combinator {
    tag("string", deferred(STRING)).into()
}

pub fn fstring() -> Combinator {
    tag("fstring", seq!(deferred(FSTRING_START), opt(seq!(choice!(deferred(fstring_replacement_field), deferred(FSTRING_MIDDLE)), repeat0(deferred(fstring_middle)))), deferred(FSTRING_END))).into()
}

pub fn fstring_format_spec() -> Combinator {
    tag("fstring_format_spec", choice!(
        deferred(FSTRING_MIDDLE),
        seq!(python_literal("{"), choice!(deferred(yield_expr), deferred(star_expressions)), opt(python_literal("=")), opt(deferred(fstring_conversion)), opt(deferred(fstring_full_format_spec)), python_literal("}"))
    )).into()
}

pub fn fstring_full_format_spec() -> Combinator {
    tag("fstring_full_format_spec", seq!(python_literal(":"), repeat0(deferred(fstring_format_spec)))).into()
}

pub fn fstring_conversion() -> Combinator {
    tag("fstring_conversion", seq!(python_literal("!"), deferred(NAME))).into()
}

pub fn fstring_replacement_field() -> Combinator {
    tag("fstring_replacement_field", seq!(
        python_literal("{"),
         choice!(deferred(yield_expr), deferred(star_expressions)),
         opt(python_literal("=")),
         opt(deferred(fstring_conversion)),
         opt(deferred(fstring_full_format_spec)),
         python_literal("}")
    )).into()
}

pub fn fstring_middle() -> Combinator {
    tag("fstring_middle", choice!(
        deferred(fstring_replacement_field),
        deferred(FSTRING_MIDDLE)
    )).into()
}

pub fn lambda_param() -> Combinator {
    tag("lambda_param", deferred(NAME)).into()
}

pub fn lambda_param_maybe_default() -> Combinator {
    tag("lambda_param_maybe_default", seq!(deferred(lambda_param), opt(seq!(python_literal("="), deferred(expression))), choice!(python_literal(","), lookahead(python_literal(":"))))).into()
}

pub fn lambda_param_with_default() -> Combinator {
    tag("lambda_param_with_default", seq!(deferred(lambda_param), python_literal("="), deferred(expression), choice!(python_literal(","), lookahead(python_literal(":"))))).into()
}

pub fn lambda_param_no_default() -> Combinator {
    tag("lambda_param_no_default", seq!(deferred(lambda_param), choice!(python_literal(","), lookahead(python_literal(":"))))).into()
}

pub fn lambda_kwds() -> Combinator {
    tag("lambda_kwds", seq!(python_literal("**"), deferred(lambda_param_no_default))).into()
}

pub fn lambda_star_etc() -> Combinator {
    tag("lambda_star_etc", choice!(
        seq!(python_literal("*"), choice!(seq!(deferred(lambda_param_no_default), repeat0(deferred(lambda_param_maybe_default)), opt(deferred(lambda_kwds))), seq!(python_literal(","), repeat1(deferred(lambda_param_maybe_default)), opt(deferred(lambda_kwds))))),
        deferred(lambda_kwds)
    )).into()
}

pub fn lambda_slash_with_default() -> Combinator {
    tag("lambda_slash_with_default", seq!(repeat0(deferred(lambda_param_no_default)), repeat1(deferred(lambda_param_with_default)), python_literal("/"), choice!(python_literal(","), lookahead(python_literal(":"))))).into()
}

pub fn lambda_slash_no_default() -> Combinator {
    tag("lambda_slash_no_default", seq!(repeat1(deferred(lambda_param_no_default)), python_literal("/"), choice!(python_literal(","), lookahead(python_literal(":"))))).into()
}

pub fn lambda_parameters() -> Combinator {
    tag("lambda_parameters", choice!(
        seq!(deferred(lambda_slash_no_default), repeat0(deferred(lambda_param_no_default)), repeat0(deferred(lambda_param_with_default)), opt(deferred(lambda_star_etc))),
        seq!(deferred(lambda_slash_with_default), repeat0(deferred(lambda_param_with_default)), opt(deferred(lambda_star_etc))),
        seq!(repeat1(deferred(lambda_param_no_default)), repeat0(deferred(lambda_param_with_default)), opt(deferred(lambda_star_etc))),
        seq!(repeat1(deferred(lambda_param_with_default)), opt(deferred(lambda_star_etc))),
        deferred(lambda_star_etc)
    )).into()
}

pub fn lambda_params() -> Combinator {
    tag("lambda_params", deferred(lambda_parameters)).into()
}

pub fn lambdef() -> Combinator {
    tag("lambdef", seq!(python_literal("lambda"), opt(deferred(lambda_params)), python_literal(":"), choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef)))).into()
}

pub fn group() -> Combinator {
    tag("group", seq!(python_literal("("), choice!(seq!(python_literal("yield"), choice!(seq!(python_literal("from"), choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef))), opt(deferred(star_expressions)))), choice!(seq!(deferred(NAME), python_literal(":="), choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef))), seq!(choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef)), negative_lookahead(python_literal(":="))))), python_literal(")"))).into()
}

pub fn atom() -> Combinator {
    tag("atom", choice!(
        deferred(NAME),
        python_literal("True"),
        python_literal("False"),
        python_literal("None"),
        seq!(lookahead(choice!(deferred(STRING), deferred(FSTRING_START))), deferred(strings)),
        deferred(NUMBER),
        seq!(lookahead(python_literal("(")), choice!(deferred(tuple), deferred(group), deferred(genexp))),
        seq!(lookahead(python_literal("[")), choice!(deferred(list), deferred(listcomp))),
        seq!(lookahead(python_literal("{")), choice!(deferred(dict), deferred(set), deferred(dictcomp), deferred(setcomp))),
        python_literal("...")
    )).into()
}

pub fn slice() -> Combinator {
    tag("slice", choice!(
        seq!(opt(choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef))), python_literal(":"), opt(choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef))), opt(seq!(python_literal(":"), opt(choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef)))))),
        choice!(seq!(deferred(NAME), python_literal(":="), choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef))), seq!(choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef)), negative_lookahead(python_literal(":="))))
    )).into()
}

pub fn slices() -> Combinator {
    tag("slices", choice!(
        seq!(deferred(slice), negative_lookahead(python_literal(","))),
        seq!(seprep1(choice!(deferred(slice), deferred(starred_expression)), python_literal(",")), opt(python_literal(",")))
    )).into()
}

pub fn primary() -> Combinator {
    tag("primary", seq!(deferred(atom), repeat0(choice!(seq!(python_literal("."), deferred(NAME)), deferred(genexp), seq!(python_literal("("), opt(deferred(arguments)), python_literal(")")), seq!(python_literal("["), deferred(slices), python_literal("]")))))).into()
}

pub fn await_primary() -> Combinator {
    cached(tag("await_primary", choice!(
        seq!(python_literal("await"), deferred(primary)),
        deferred(primary)
    ))).into()
}

pub fn power() -> Combinator {
    tag("power", seq!(deferred(await_primary), opt(seq!(python_literal("**"), choice!(seq!(python_literal("+"), deferred(factor)), seq!(python_literal("-"), deferred(factor)), seq!(python_literal("~"), deferred(factor)), deferred(power)))))).into()
}

pub fn factor() -> Combinator {
    cached(tag("factor", choice!(
        seq!(python_literal("+"), deferred(factor)),
        seq!(python_literal("-"), deferred(factor)),
        seq!(python_literal("~"), deferred(factor)),
        deferred(power)
    ))).into()
}

pub fn term() -> Combinator {
    tag("term", seq!(deferred(factor), repeat0(choice!(seq!(python_literal("*"), deferred(factor)), seq!(python_literal("/"), deferred(factor)), seq!(python_literal("//"), deferred(factor)), seq!(python_literal("%"), deferred(factor)), seq!(python_literal("@"), deferred(factor)))))).into()
}

pub fn sum() -> Combinator {
    tag("sum", seq!(deferred(term), repeat0(choice!(seq!(python_literal("+"), deferred(term)), seq!(python_literal("-"), deferred(term)))))).into()
}

pub fn shift_expr() -> Combinator {
    tag("shift_expr", seq!(deferred(sum), repeat0(choice!(seq!(python_literal("<<"), deferred(sum)), seq!(python_literal(">>"), deferred(sum)))))).into()
}

pub fn bitwise_and() -> Combinator {
    tag("bitwise_and", seq!(deferred(shift_expr), repeat0(seq!(python_literal("&"), deferred(shift_expr))))).into()
}

pub fn bitwise_xor() -> Combinator {
    tag("bitwise_xor", seq!(deferred(bitwise_and), repeat0(seq!(python_literal("^"), deferred(bitwise_and))))).into()
}

pub fn bitwise_or() -> Combinator {
    tag("bitwise_or", seq!(deferred(bitwise_xor), repeat0(seq!(python_literal("|"), deferred(bitwise_xor))))).into()
}

pub fn is_bitwise_or() -> Combinator {
    tag("is_bitwise_or", seq!(python_literal("is"), deferred(bitwise_or))).into()
}

pub fn isnot_bitwise_or() -> Combinator {
    tag("isnot_bitwise_or", seq!(python_literal("is"), python_literal("not"), deferred(bitwise_or))).into()
}

pub fn in_bitwise_or() -> Combinator {
    tag("in_bitwise_or", seq!(python_literal("in"), deferred(bitwise_or))).into()
}

pub fn notin_bitwise_or() -> Combinator {
    tag("notin_bitwise_or", seq!(python_literal("not"), python_literal("in"), deferred(bitwise_or))).into()
}

pub fn gt_bitwise_or() -> Combinator {
    tag("gt_bitwise_or", seq!(python_literal(">"), deferred(bitwise_or))).into()
}

pub fn gte_bitwise_or() -> Combinator {
    tag("gte_bitwise_or", seq!(python_literal(">="), deferred(bitwise_or))).into()
}

pub fn lt_bitwise_or() -> Combinator {
    tag("lt_bitwise_or", seq!(python_literal("<"), deferred(bitwise_or))).into()
}

pub fn lte_bitwise_or() -> Combinator {
    tag("lte_bitwise_or", seq!(python_literal("<="), deferred(bitwise_or))).into()
}

pub fn noteq_bitwise_or() -> Combinator {
    tag("noteq_bitwise_or", seq!(python_literal("!="), deferred(bitwise_or))).into()
}

pub fn eq_bitwise_or() -> Combinator {
    tag("eq_bitwise_or", seq!(python_literal("=="), deferred(bitwise_or))).into()
}

pub fn compare_op_bitwise_or_pair() -> Combinator {
    tag("compare_op_bitwise_or_pair", choice!(
        deferred(eq_bitwise_or),
        deferred(noteq_bitwise_or),
        deferred(lte_bitwise_or),
        deferred(lt_bitwise_or),
        deferred(gte_bitwise_or),
        deferred(gt_bitwise_or),
        deferred(notin_bitwise_or),
        deferred(in_bitwise_or),
        deferred(isnot_bitwise_or),
        deferred(is_bitwise_or)
    )).into()
}

pub fn comparison() -> Combinator {
    tag("comparison", seq!(deferred(bitwise_or), repeat0(deferred(compare_op_bitwise_or_pair)))).into()
}

pub fn inversion() -> Combinator {
    cached(tag("inversion", choice!(
        seq!(python_literal("not"), deferred(inversion)),
        deferred(comparison)
    ))).into()
}

pub fn conjunction() -> Combinator {
    cached(tag("conjunction", seq!(deferred(inversion), repeat0(seq!(python_literal("and"), deferred(inversion)))))).into()
}

pub fn disjunction() -> Combinator {
    cached(tag("disjunction", seq!(deferred(conjunction), repeat0(seq!(python_literal("or"), deferred(conjunction)))))).into()
}

pub fn named_expression() -> Combinator {
    tag("named_expression", choice!(
        seq!(deferred(NAME), python_literal(":="), choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef))),
        seq!(choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef)), negative_lookahead(python_literal(":=")))
    )).into()
}

pub fn assignment_expression() -> Combinator {
    tag("assignment_expression", seq!(deferred(NAME), python_literal(":="), choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef)))).into()
}

pub fn star_named_expression() -> Combinator {
    tag("star_named_expression", choice!(
        seq!(python_literal("*"), deferred(bitwise_or)),
        deferred(named_expression)
    )).into()
}

pub fn star_named_expressions() -> Combinator {
    tag("star_named_expressions", seq!(seprep1(deferred(star_named_expression), python_literal(",")), opt(python_literal(",")))).into()
}

pub fn star_expression() -> Combinator {
    cached(tag("star_expression", choice!(
        seq!(python_literal("*"), deferred(bitwise_or)),
        choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef))
    ))).into()
}

pub fn star_expressions() -> Combinator {
    tag("star_expressions", seq!(deferred(star_expression), opt(choice!(seq!(repeat1(seq!(python_literal(","), deferred(star_expression))), opt(python_literal(","))), python_literal(","))))).into()
}

pub fn yield_expr() -> Combinator {
    tag("yield_expr", seq!(python_literal("yield"), choice!(seq!(python_literal("from"), choice!(seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))), deferred(lambdef))), opt(deferred(star_expressions))))).into()
}

pub fn expression() -> Combinator {
    cached(tag("expression", choice!(
        seq!(deferred(disjunction), opt(seq!(python_literal("if"), deferred(disjunction), python_literal("else"), deferred(expression)))),
        deferred(lambdef)
    ))).into()
}

pub fn expressions() -> Combinator {
    tag("expressions", seq!(deferred(expression), opt(choice!(seq!(repeat1(seq!(python_literal(","), deferred(expression))), opt(python_literal(","))), python_literal(","))))).into()
}

pub fn type_param_starred_default() -> Combinator {
    tag("type_param_starred_default", seq!(python_literal("="), deferred(star_expression))).into()
}

pub fn type_param_default() -> Combinator {
    tag("type_param_default", seq!(python_literal("="), deferred(expression))).into()
}

pub fn type_param_bound() -> Combinator {
    tag("type_param_bound", seq!(python_literal(":"), deferred(expression))).into()
}

pub fn type_param() -> Combinator {
    cached(tag("type_param", choice!(
        seq!(deferred(NAME), opt(deferred(type_param_bound)), opt(deferred(type_param_default))),
        seq!(python_literal("*"), deferred(NAME), opt(deferred(type_param_starred_default))),
        seq!(python_literal("**"), deferred(NAME), opt(deferred(type_param_default)))
    ))).into()
}

pub fn type_param_seq() -> Combinator {
    tag("type_param_seq", seq!(seprep1(deferred(type_param), python_literal(",")), opt(python_literal(",")))).into()
}

pub fn type_params() -> Combinator {
    tag("type_params", seq!(python_literal("["), deferred(type_param_seq), python_literal("]"))).into()
}

pub fn type_alias() -> Combinator {
    tag("type_alias", seq!(
        python_literal("type"),
         deferred(NAME),
         opt(deferred(type_params)),
         python_literal("="),
         deferred(expression)
    )).into()
}

pub fn keyword_pattern() -> Combinator {
    tag("keyword_pattern", seq!(deferred(NAME), python_literal("="), choice!(deferred(as_pattern), deferred(or_pattern)))).into()
}

pub fn keyword_patterns() -> Combinator {
    tag("keyword_patterns", seprep1(deferred(keyword_pattern), python_literal(","))).into()
}

pub fn positional_patterns() -> Combinator {
    tag("positional_patterns", seprep1(deferred(pattern), python_literal(","))).into()
}

pub fn class_pattern() -> Combinator {
    tag("class_pattern", seq!(deferred(NAME), repeat0(seq!(python_literal("."), deferred(NAME))), python_literal("("), choice!(python_literal(")"), seq!(deferred(positional_patterns), choice!(seq!(opt(python_literal(",")), python_literal(")")), seq!(python_literal(","), deferred(keyword_patterns), opt(python_literal(",")), python_literal(")")))), seq!(deferred(keyword_patterns), opt(python_literal(",")), python_literal(")"))))).into()
}

pub fn double_star_pattern() -> Combinator {
    tag("double_star_pattern", seq!(python_literal("**"), negative_lookahead(python_literal("_")), deferred(NAME), negative_lookahead(choice!(python_literal("."), python_literal("("), python_literal("="))))).into()
}

pub fn key_value_pattern() -> Combinator {
    tag("key_value_pattern", seq!(choice!(choice!(seq!(deferred(signed_number), negative_lookahead(choice!(python_literal("+"), python_literal("-")))), deferred(complex_number), deferred(strings), python_literal("None"), python_literal("True"), python_literal("False")), seq!(deferred(name_or_attr), python_literal("."), deferred(NAME))), python_literal(":"), choice!(deferred(as_pattern), deferred(or_pattern)))).into()
}

pub fn items_pattern() -> Combinator {
    tag("items_pattern", seprep1(deferred(key_value_pattern), python_literal(","))).into()
}

pub fn mapping_pattern() -> Combinator {
    tag("mapping_pattern", seq!(python_literal("{"), choice!(python_literal("}"), seq!(deferred(double_star_pattern), opt(python_literal(",")), python_literal("}")), seq!(deferred(items_pattern), choice!(seq!(python_literal(","), deferred(double_star_pattern), opt(python_literal(",")), python_literal("}")), seq!(opt(python_literal(",")), python_literal("}"))))))).into()
}

pub fn star_pattern() -> Combinator {
    cached(tag("star_pattern", seq!(python_literal("*"), choice!(seq!(negative_lookahead(python_literal("_")), deferred(NAME), negative_lookahead(choice!(python_literal("."), python_literal("("), python_literal("=")))), python_literal("_"))))).into()
}

pub fn maybe_star_pattern() -> Combinator {
    tag("maybe_star_pattern", choice!(
        deferred(star_pattern),
        choice!(deferred(as_pattern), deferred(or_pattern))
    )).into()
}

pub fn maybe_sequence_pattern() -> Combinator {
    tag("maybe_sequence_pattern", seq!(seprep1(deferred(maybe_star_pattern), python_literal(",")), opt(python_literal(",")))).into()
}

pub fn open_sequence_pattern() -> Combinator {
    tag("open_sequence_pattern", seq!(deferred(maybe_star_pattern), python_literal(","), opt(deferred(maybe_sequence_pattern)))).into()
}

pub fn sequence_pattern() -> Combinator {
    tag("sequence_pattern", choice!(
        seq!(python_literal("["), opt(deferred(maybe_sequence_pattern)), python_literal("]")),
        seq!(python_literal("("), opt(deferred(open_sequence_pattern)), python_literal(")"))
    )).into()
}

pub fn group_pattern() -> Combinator {
    tag("group_pattern", seq!(python_literal("("), choice!(deferred(as_pattern), deferred(or_pattern)), python_literal(")"))).into()
}

pub fn name_or_attr() -> Combinator {
    tag("name_or_attr", seq!(deferred(NAME), repeat0(seq!(python_literal("."), deferred(NAME))))).into()
}

pub fn attr() -> Combinator {
    tag("attr", seq!(deferred(name_or_attr), python_literal("."), deferred(NAME))).into()
}

pub fn value_pattern() -> Combinator {
    tag("value_pattern", seq!(deferred(attr), negative_lookahead(choice!(python_literal("."), python_literal("("), python_literal("="))))).into()
}

pub fn wildcard_pattern() -> Combinator {
    tag("wildcard_pattern", python_literal("_")).into()
}

pub fn pattern_capture_target() -> Combinator {
    tag("pattern_capture_target", seq!(negative_lookahead(python_literal("_")), deferred(NAME), negative_lookahead(choice!(python_literal("."), python_literal("("), python_literal("="))))).into()
}

pub fn capture_pattern() -> Combinator {
    tag("capture_pattern", deferred(pattern_capture_target)).into()
}

pub fn imaginary_number() -> Combinator {
    tag("imaginary_number", deferred(NUMBER)).into()
}

pub fn real_number() -> Combinator {
    tag("real_number", deferred(NUMBER)).into()
}

pub fn signed_real_number() -> Combinator {
    tag("signed_real_number", choice!(
        deferred(real_number),
        seq!(python_literal("-"), deferred(real_number))
    )).into()
}

pub fn signed_number() -> Combinator {
    tag("signed_number", choice!(
        deferred(NUMBER),
        seq!(python_literal("-"), deferred(NUMBER))
    )).into()
}

pub fn complex_number() -> Combinator {
    tag("complex_number", seq!(deferred(signed_real_number), choice!(seq!(python_literal("+"), deferred(imaginary_number)), seq!(python_literal("-"), deferred(imaginary_number))))).into()
}

pub fn literal_expr() -> Combinator {
    tag("literal_expr", choice!(
        seq!(deferred(signed_number), negative_lookahead(choice!(python_literal("+"), python_literal("-")))),
        deferred(complex_number),
        deferred(strings),
        python_literal("None"),
        python_literal("True"),
        python_literal("False")
    )).into()
}

pub fn literal_pattern() -> Combinator {
    tag("literal_pattern", choice!(
        seq!(deferred(signed_number), negative_lookahead(choice!(python_literal("+"), python_literal("-")))),
        deferred(complex_number),
        deferred(strings),
        python_literal("None"),
        python_literal("True"),
        python_literal("False")
    )).into()
}

pub fn closed_pattern() -> Combinator {
    cached(tag("closed_pattern", choice!(
        deferred(literal_pattern),
        deferred(capture_pattern),
        deferred(wildcard_pattern),
        deferred(value_pattern),
        deferred(group_pattern),
        deferred(sequence_pattern),
        deferred(mapping_pattern),
        deferred(class_pattern)
    ))).into()
}

pub fn or_pattern() -> Combinator {
    tag("or_pattern", seprep1(deferred(closed_pattern), python_literal("|"))).into()
}

pub fn as_pattern() -> Combinator {
    tag("as_pattern", seq!(deferred(or_pattern), python_literal("as"), deferred(pattern_capture_target))).into()
}

pub fn pattern() -> Combinator {
    tag("pattern", choice!(
        deferred(as_pattern),
        deferred(or_pattern)
    )).into()
}

pub fn patterns() -> Combinator {
    tag("patterns", choice!(
        deferred(open_sequence_pattern),
        deferred(pattern)
    )).into()
}

pub fn guard() -> Combinator {
    tag("guard", seq!(python_literal("if"), deferred(named_expression))).into()
}

pub fn case_block() -> Combinator {
    tag("case_block", seq!(
        python_literal("case"),
         deferred(patterns),
         opt(deferred(guard)),
         python_literal(":"),
         choice!(seq!(deferred(NEWLINE), deferred(INDENT), repeat1(deferred(statement)), deferred(DEDENT)), choice!(seq!(deferred(simple_stmt), negative_lookahead(python_literal(";")), deferred(NEWLINE)), seq!(seprep1(deferred(simple_stmt), python_literal(";")), opt(python_literal(";")), deferred(NEWLINE))))
    )).into()
}

pub fn subject_expr() -> Combinator {
    tag("subject_expr", choice!(
        seq!(deferred(star_named_expression), python_literal(","), opt(deferred(star_named_expressions))),
        deferred(named_expression)
    )).into()
}

pub fn match_stmt() -> Combinator {
    tag("match_stmt", seq!(
        python_literal("match"),
         deferred(subject_expr),
         python_literal(":"),
         deferred(NEWLINE),
         deferred(INDENT),
         repeat1(deferred(case_block)),
         deferred(DEDENT)
    )).into()
}

pub fn finally_block() -> Combinator {
    tag("finally_block", seq!(python_literal("finally"), python_literal(":"), choice!(seq!(deferred(NEWLINE), deferred(INDENT), repeat1(deferred(statement)), deferred(DEDENT)), choice!(seq!(deferred(simple_stmt), negative_lookahead(python_literal(";")), deferred(NEWLINE)), seq!(seprep1(deferred(simple_stmt), python_literal(";")), opt(python_literal(";")), deferred(NEWLINE)))))).into()
}

pub fn except_star_block() -> Combinator {
    tag("except_star_block", seq!(
        python_literal("except"),
         python_literal("*"),
         deferred(expression),
         opt(seq!(python_literal("as"), deferred(NAME))),
         python_literal(":"),
         choice!(seq!(deferred(NEWLINE), deferred(INDENT), repeat1(deferred(statement)), deferred(DEDENT)), choice!(seq!(deferred(simple_stmt), negative_lookahead(python_literal(";")), deferred(NEWLINE)), seq!(seprep1(deferred(simple_stmt), python_literal(";")), opt(python_literal(";")), deferred(NEWLINE))))
    )).into()
}

pub fn except_block() -> Combinator {
    tag("except_block", seq!(python_literal("except"), choice!(seq!(deferred(expression), opt(seq!(python_literal("as"), deferred(NAME))), python_literal(":"), choice!(seq!(deferred(NEWLINE), deferred(INDENT), repeat1(deferred(statement)), deferred(DEDENT)), choice!(seq!(deferred(simple_stmt), negative_lookahead(python_literal(";")), deferred(NEWLINE)), seq!(seprep1(deferred(simple_stmt), python_literal(";")), opt(python_literal(";")), deferred(NEWLINE))))), seq!(python_literal(":"), choice!(seq!(deferred(NEWLINE), deferred(INDENT), repeat1(deferred(statement)), deferred(DEDENT)), choice!(seq!(deferred(simple_stmt), negative_lookahead(python_literal(";")), deferred(NEWLINE)), seq!(seprep1(deferred(simple_stmt), python_literal(";")), opt(python_literal(";")), deferred(NEWLINE)))))))).into()
}

pub fn try_stmt() -> Combinator {
    tag("try_stmt", seq!(python_literal("try"), python_literal(":"), choice!(seq!(deferred(NEWLINE), deferred(INDENT), repeat1(deferred(statement)), deferred(DEDENT)), choice!(seq!(deferred(simple_stmt), negative_lookahead(python_literal(";")), deferred(NEWLINE)), seq!(seprep1(deferred(simple_stmt), python_literal(";")), opt(python_literal(";")), deferred(NEWLINE)))), choice!(deferred(finally_block), seq!(repeat1(deferred(except_block)), opt(seq!(python_literal("else"), python_literal(":"), choice!(seq!(deferred(NEWLINE), deferred(INDENT), repeat1(deferred(statement)), deferred(DEDENT)), choice!(seq!(deferred(simple_stmt), negative_lookahead(python_literal(";")), deferred(NEWLINE)), seq!(seprep1(deferred(simple_stmt), python_literal(";")), opt(python_literal(";")), deferred(NEWLINE)))))), opt(deferred(finally_block))), seq!(repeat1(deferred(except_star_block)), opt(seq!(python_literal("else"), python_literal(":"), choice!(seq!(deferred(NEWLINE), deferred(INDENT), repeat1(deferred(statement)), deferred(DEDENT)), choice!(seq!(deferred(simple_stmt), negative_lookahead(python_literal(";")), deferred(NEWLINE)), seq!(seprep1(deferred(simple_stmt), python_literal(";")), opt(python_literal(";")), deferred(NEWLINE)))))), opt(deferred(finally_block)))))).into()
}

pub fn with_item() -> Combinator {
    tag("with_item", seq!(deferred(expression), opt(seq!(python_literal("as"), deferred(star_target), lookahead(choice!(python_literal(","), python_literal(")"), python_literal(":"))))))).into()
}

pub fn with_stmt() -> Combinator {
    tag("with_stmt", choice!(
        seq!(python_literal("with"), choice!(seq!(python_literal("("), seprep1(deferred(with_item), python_literal(",")), opt(python_literal(",")), python_literal(")"), python_literal(":"), opt(deferred(TYPE_COMMENT)), choice!(seq!(deferred(NEWLINE), deferred(INDENT), repeat1(deferred(statement)), deferred(DEDENT)), choice!(seq!(deferred(simple_stmt), negative_lookahead(python_literal(";")), deferred(NEWLINE)), seq!(seprep1(deferred(simple_stmt), python_literal(";")), opt(python_literal(";")), deferred(NEWLINE))))), seq!(seprep1(deferred(with_item), python_literal(",")), python_literal(":"), opt(deferred(TYPE_COMMENT)), choice!(seq!(deferred(NEWLINE), deferred(INDENT), repeat1(deferred(statement)), deferred(DEDENT)), choice!(seq!(deferred(simple_stmt), negative_lookahead(python_literal(";")), deferred(NEWLINE)), seq!(seprep1(deferred(simple_stmt), python_literal(";")), opt(python_literal(";")), deferred(NEWLINE))))))),
        seq!(python_literal("async"), python_literal("with"), choice!(seq!(python_literal("("), seprep1(deferred(with_item), python_literal(",")), opt(python_literal(",")), python_literal(")"), python_literal(":"), choice!(seq!(deferred(NEWLINE), deferred(INDENT), repeat1(deferred(statement)), deferred(DEDENT)), choice!(seq!(deferred(simple_stmt), negative_lookahead(python_literal(";")), deferred(NEWLINE)), seq!(seprep1(deferred(simple_stmt), python_literal(";")), opt(python_literal(";")), deferred(NEWLINE))))), seq!(seprep1(deferred(with_item), python_literal(",")), python_literal(":"), opt(deferred(TYPE_COMMENT)), choice!(seq!(deferred(NEWLINE), deferred(INDENT), repeat1(deferred(statement)), deferred(DEDENT)), choice!(seq!(deferred(simple_stmt), negative_lookahead(python_literal(";")), deferred(NEWLINE)), seq!(seprep1(deferred(simple_stmt), python_literal(";")), opt(python_literal(";")), deferred(NEWLINE)))))))
    )).into()
}

pub fn for_stmt() -> Combinator {
    tag("for_stmt", choice!(
        seq!(python_literal("for"), deferred(star_targets), python_literal("in"), deferred(star_expressions), python_literal(":"), opt(deferred(TYPE_COMMENT)), choice!(seq!(deferred(NEWLINE), deferred(INDENT), repeat1(deferred(statement)), deferred(DEDENT)), choice!(seq!(deferred(simple_stmt), negative_lookahead(python_literal(";")), deferred(NEWLINE)), seq!(seprep1(deferred(simple_stmt), python_literal(";")), opt(python_literal(";")), deferred(NEWLINE)))), opt(seq!(python_literal("else"), python_literal(":"), choice!(seq!(deferred(NEWLINE), deferred(INDENT), repeat1(deferred(statement)), deferred(DEDENT)), choice!(seq!(deferred(simple_stmt), negative_lookahead(python_literal(";")), deferred(NEWLINE)), seq!(seprep1(deferred(simple_stmt), python_literal(";")), opt(python_literal(";")), deferred(NEWLINE))))))),
        seq!(python_literal("async"), python_literal("for"), deferred(star_targets), python_literal("in"), deferred(star_expressions), python_literal(":"), opt(deferred(TYPE_COMMENT)), choice!(seq!(deferred(NEWLINE), deferred(INDENT), repeat1(deferred(statement)), deferred(DEDENT)), choice!(seq!(deferred(simple_stmt), negative_lookahead(python_literal(";")), deferred(NEWLINE)), seq!(seprep1(deferred(simple_stmt), python_literal(";")), opt(python_literal(";")), deferred(NEWLINE)))), opt(seq!(python_literal("else"), python_literal(":"), choice!(seq!(deferred(NEWLINE), deferred(INDENT), repeat1(deferred(statement)), deferred(DEDENT)), choice!(seq!(deferred(simple_stmt), negative_lookahead(python_literal(";")), deferred(NEWLINE)), seq!(seprep1(deferred(simple_stmt), python_literal(";")), opt(python_literal(";")), deferred(NEWLINE)))))))
    )).into()
}

pub fn while_stmt() -> Combinator {
    tag("while_stmt", seq!(
        python_literal("while"),
         deferred(named_expression),
         python_literal(":"),
         choice!(seq!(deferred(NEWLINE), deferred(INDENT), repeat1(deferred(statement)), deferred(DEDENT)), choice!(seq!(deferred(simple_stmt), negative_lookahead(python_literal(";")), deferred(NEWLINE)), seq!(seprep1(deferred(simple_stmt), python_literal(";")), opt(python_literal(";")), deferred(NEWLINE)))),
         opt(seq!(python_literal("else"), python_literal(":"), choice!(seq!(deferred(NEWLINE), deferred(INDENT), repeat1(deferred(statement)), deferred(DEDENT)), choice!(seq!(deferred(simple_stmt), negative_lookahead(python_literal(";")), deferred(NEWLINE)), seq!(seprep1(deferred(simple_stmt), python_literal(";")), opt(python_literal(";")), deferred(NEWLINE))))))
    )).into()
}

pub fn else_block() -> Combinator {
    tag("else_block", seq!(python_literal("else"), python_literal(":"), choice!(seq!(deferred(NEWLINE), deferred(INDENT), repeat1(deferred(statement)), deferred(DEDENT)), choice!(seq!(deferred(simple_stmt), negative_lookahead(python_literal(";")), deferred(NEWLINE)), seq!(seprep1(deferred(simple_stmt), python_literal(";")), opt(python_literal(";")), deferred(NEWLINE)))))).into()
}

pub fn elif_stmt() -> Combinator {
    tag("elif_stmt", seq!(
        python_literal("elif"),
         deferred(named_expression),
         python_literal(":"),
         choice!(seq!(deferred(NEWLINE), deferred(INDENT), repeat1(deferred(statement)), deferred(DEDENT)), choice!(seq!(deferred(simple_stmt), negative_lookahead(python_literal(";")), deferred(NEWLINE)), seq!(seprep1(deferred(simple_stmt), python_literal(";")), opt(python_literal(";")), deferred(NEWLINE)))),
         choice!(deferred(elif_stmt), opt(deferred(else_block)))
    )).into()
}

pub fn if_stmt() -> Combinator {
    tag("if_stmt", seq!(
        python_literal("if"),
         deferred(named_expression),
         python_literal(":"),
         choice!(seq!(deferred(NEWLINE), deferred(INDENT), repeat1(deferred(statement)), deferred(DEDENT)), choice!(seq!(deferred(simple_stmt), negative_lookahead(python_literal(";")), deferred(NEWLINE)), seq!(seprep1(deferred(simple_stmt), python_literal(";")), opt(python_literal(";")), deferred(NEWLINE)))),
         choice!(deferred(elif_stmt), opt(deferred(else_block)))
    )).into()
}

pub fn default() -> Combinator {
    tag("default", seq!(python_literal("="), deferred(expression))).into()
}

pub fn star_annotation() -> Combinator {
    tag("star_annotation", seq!(python_literal(":"), deferred(star_expression))).into()
}

pub fn annotation() -> Combinator {
    tag("annotation", seq!(python_literal(":"), deferred(expression))).into()
}

pub fn param_star_annotation() -> Combinator {
    tag("param_star_annotation", seq!(deferred(NAME), deferred(star_annotation))).into()
}

pub fn param() -> Combinator {
    tag("param", seq!(deferred(NAME), opt(deferred(annotation)))).into()
}

pub fn param_maybe_default() -> Combinator {
    tag("param_maybe_default", seq!(deferred(param), opt(deferred(default)), choice!(seq!(python_literal(","), opt(deferred(TYPE_COMMENT))), seq!(opt(deferred(TYPE_COMMENT)), lookahead(python_literal(")")))))).into()
}

pub fn param_with_default() -> Combinator {
    tag("param_with_default", seq!(deferred(param), deferred(default), choice!(seq!(python_literal(","), opt(deferred(TYPE_COMMENT))), seq!(opt(deferred(TYPE_COMMENT)), lookahead(python_literal(")")))))).into()
}

pub fn param_no_default_star_annotation() -> Combinator {
    tag("param_no_default_star_annotation", seq!(deferred(param_star_annotation), choice!(seq!(python_literal(","), opt(deferred(TYPE_COMMENT))), seq!(opt(deferred(TYPE_COMMENT)), lookahead(python_literal(")")))))).into()
}

pub fn param_no_default() -> Combinator {
    tag("param_no_default", seq!(deferred(param), choice!(seq!(python_literal(","), opt(deferred(TYPE_COMMENT))), seq!(opt(deferred(TYPE_COMMENT)), lookahead(python_literal(")")))))).into()
}

pub fn kwds() -> Combinator {
    tag("kwds", seq!(python_literal("**"), deferred(param_no_default))).into()
}

pub fn star_etc() -> Combinator {
    tag("star_etc", choice!(
        seq!(python_literal("*"), choice!(seq!(deferred(param_no_default), repeat0(deferred(param_maybe_default)), opt(deferred(kwds))), seq!(deferred(param_no_default_star_annotation), repeat0(deferred(param_maybe_default)), opt(deferred(kwds))), seq!(python_literal(","), repeat1(deferred(param_maybe_default)), opt(deferred(kwds))))),
        deferred(kwds)
    )).into()
}

pub fn slash_with_default() -> Combinator {
    tag("slash_with_default", seq!(repeat0(deferred(param_no_default)), repeat1(deferred(param_with_default)), python_literal("/"), choice!(python_literal(","), lookahead(python_literal(")"))))).into()
}

pub fn slash_no_default() -> Combinator {
    tag("slash_no_default", seq!(repeat1(deferred(param_no_default)), python_literal("/"), choice!(python_literal(","), lookahead(python_literal(")"))))).into()
}

pub fn parameters() -> Combinator {
    tag("parameters", choice!(
        seq!(deferred(slash_no_default), repeat0(deferred(param_no_default)), repeat0(deferred(param_with_default)), opt(deferred(star_etc))),
        seq!(deferred(slash_with_default), repeat0(deferred(param_with_default)), opt(deferred(star_etc))),
        seq!(repeat1(deferred(param_no_default)), repeat0(deferred(param_with_default)), opt(deferred(star_etc))),
        seq!(repeat1(deferred(param_with_default)), opt(deferred(star_etc))),
        deferred(star_etc)
    )).into()
}

pub fn params() -> Combinator {
    tag("params", deferred(parameters)).into()
}

pub fn function_def_raw() -> Combinator {
    tag("function_def_raw", choice!(
        seq!(python_literal("def"), deferred(NAME), opt(deferred(type_params)), python_literal("("), opt(deferred(params)), python_literal(")"), opt(seq!(python_literal("->"), deferred(expression))), python_literal(":"), opt(deferred(func_type_comment)), choice!(seq!(deferred(NEWLINE), deferred(INDENT), repeat1(deferred(statement)), deferred(DEDENT)), choice!(seq!(deferred(simple_stmt), negative_lookahead(python_literal(";")), deferred(NEWLINE)), seq!(seprep1(deferred(simple_stmt), python_literal(";")), opt(python_literal(";")), deferred(NEWLINE))))),
        seq!(python_literal("async"), python_literal("def"), deferred(NAME), opt(deferred(type_params)), python_literal("("), opt(deferred(params)), python_literal(")"), opt(seq!(python_literal("->"), deferred(expression))), python_literal(":"), opt(deferred(func_type_comment)), choice!(seq!(deferred(NEWLINE), deferred(INDENT), repeat1(deferred(statement)), deferred(DEDENT)), choice!(seq!(deferred(simple_stmt), negative_lookahead(python_literal(";")), deferred(NEWLINE)), seq!(seprep1(deferred(simple_stmt), python_literal(";")), opt(python_literal(";")), deferred(NEWLINE)))))
    )).into()
}

pub fn function_def() -> Combinator {
    tag("function_def", choice!(
        seq!(repeat1(seq!(python_literal("@"), deferred(named_expression), deferred(NEWLINE))), deferred(function_def_raw)),
        deferred(function_def_raw)
    )).into()
}

pub fn class_def_raw() -> Combinator {
    tag("class_def_raw", seq!(
        python_literal("class"),
         deferred(NAME),
         opt(deferred(type_params)),
         opt(seq!(python_literal("("), opt(deferred(arguments)), python_literal(")"))),
         python_literal(":"),
         choice!(seq!(deferred(NEWLINE), deferred(INDENT), repeat1(deferred(statement)), deferred(DEDENT)), choice!(seq!(deferred(simple_stmt), negative_lookahead(python_literal(";")), deferred(NEWLINE)), seq!(seprep1(deferred(simple_stmt), python_literal(";")), opt(python_literal(";")), deferred(NEWLINE))))
    )).into()
}

pub fn class_def() -> Combinator {
    tag("class_def", choice!(
        seq!(repeat1(seq!(python_literal("@"), deferred(named_expression), deferred(NEWLINE))), deferred(class_def_raw)),
        deferred(class_def_raw)
    )).into()
}

pub fn decorators() -> Combinator {
    tag("decorators", repeat1(seq!(python_literal("@"), deferred(named_expression), deferred(NEWLINE)))).into()
}

pub fn block() -> Combinator {
    cached(tag("block", choice!(
        seq!(deferred(NEWLINE), deferred(INDENT), repeat1(deferred(statement)), deferred(DEDENT)),
        choice!(seq!(deferred(simple_stmt), negative_lookahead(python_literal(";")), deferred(NEWLINE)), seq!(seprep1(deferred(simple_stmt), python_literal(";")), opt(python_literal(";")), deferred(NEWLINE)))
    ))).into()
}

pub fn dotted_name() -> Combinator {
    tag("dotted_name", seq!(deferred(NAME), repeat0(seq!(python_literal("."), deferred(NAME))))).into()
}

pub fn dotted_as_name() -> Combinator {
    tag("dotted_as_name", seq!(deferred(dotted_name), opt(seq!(python_literal("as"), deferred(NAME))))).into()
}

pub fn dotted_as_names() -> Combinator {
    tag("dotted_as_names", seprep1(deferred(dotted_as_name), python_literal(","))).into()
}

pub fn import_from_as_name() -> Combinator {
    tag("import_from_as_name", seq!(deferred(NAME), opt(seq!(python_literal("as"), deferred(NAME))))).into()
}

pub fn import_from_as_names() -> Combinator {
    tag("import_from_as_names", seprep1(deferred(import_from_as_name), python_literal(","))).into()
}

pub fn import_from_targets() -> Combinator {
    tag("import_from_targets", choice!(
        seq!(python_literal("("), deferred(import_from_as_names), opt(python_literal(",")), python_literal(")")),
        seq!(deferred(import_from_as_names), negative_lookahead(python_literal(","))),
        python_literal("*")
    )).into()
}

pub fn import_from() -> Combinator {
    tag("import_from", seq!(python_literal("from"), choice!(seq!(repeat0(choice!(python_literal("."), python_literal("..."))), deferred(dotted_name), python_literal("import"), deferred(import_from_targets)), seq!(repeat1(choice!(python_literal("."), python_literal("..."))), python_literal("import"), deferred(import_from_targets))))).into()
}

pub fn import_name() -> Combinator {
    tag("import_name", seq!(python_literal("import"), deferred(dotted_as_names))).into()
}

pub fn import_stmt() -> Combinator {
    tag("import_stmt", choice!(
        deferred(import_name),
        deferred(import_from)
    )).into()
}

pub fn assert_stmt() -> Combinator {
    tag("assert_stmt", seq!(python_literal("assert"), deferred(expression), opt(seq!(python_literal(","), deferred(expression))))).into()
}

pub fn yield_stmt() -> Combinator {
    tag("yield_stmt", deferred(yield_expr)).into()
}

pub fn del_stmt() -> Combinator {
    tag("del_stmt", seq!(python_literal("del"), deferred(del_targets), lookahead(choice!(python_literal(";"), deferred(NEWLINE))))).into()
}

pub fn nonlocal_stmt() -> Combinator {
    tag("nonlocal_stmt", seq!(python_literal("nonlocal"), seprep1(deferred(NAME), python_literal(",")))).into()
}

pub fn global_stmt() -> Combinator {
    tag("global_stmt", seq!(python_literal("global"), seprep1(deferred(NAME), python_literal(",")))).into()
}

pub fn raise_stmt() -> Combinator {
    tag("raise_stmt", seq!(python_literal("raise"), opt(seq!(deferred(expression), opt(seq!(python_literal("from"), deferred(expression))))))).into()
}

pub fn return_stmt() -> Combinator {
    tag("return_stmt", seq!(python_literal("return"), opt(deferred(star_expressions)))).into()
}

pub fn augassign() -> Combinator {
    tag("augassign", choice!(
        python_literal("+="),
        python_literal("-="),
        python_literal("*="),
        python_literal("@="),
        python_literal("/="),
        python_literal("%="),
        python_literal("&="),
        python_literal("|="),
        python_literal("^="),
        python_literal("<<="),
        python_literal(">>="),
        python_literal("**="),
        python_literal("//=")
    )).into()
}

pub fn annotated_rhs() -> Combinator {
    tag("annotated_rhs", choice!(
        deferred(yield_expr),
        deferred(star_expressions)
    )).into()
}

pub fn assignment() -> Combinator {
    tag("assignment", choice!(
        seq!(deferred(NAME), python_literal(":"), deferred(expression), opt(seq!(python_literal("="), deferred(annotated_rhs)))),
        seq!(choice!(seq!(python_literal("("), deferred(single_target), python_literal(")")), deferred(single_subscript_attribute_target)), python_literal(":"), deferred(expression), opt(seq!(python_literal("="), deferred(annotated_rhs)))),
        seq!(repeat1(seq!(deferred(star_targets), python_literal("="))), choice!(deferred(yield_expr), deferred(star_expressions)), negative_lookahead(python_literal("=")), opt(deferred(TYPE_COMMENT))),
        seq!(deferred(single_target), deferred(augassign), choice!(deferred(yield_expr), deferred(star_expressions)))
    )).into()
}

pub fn compound_stmt() -> Combinator {
    tag("compound_stmt", choice!(
        seq!(lookahead(choice!(python_literal("def"), python_literal("@"), python_literal("async"))), deferred(function_def)),
        seq!(lookahead(python_literal("if")), deferred(if_stmt)),
        seq!(lookahead(choice!(python_literal("class"), python_literal("@"))), deferred(class_def)),
        seq!(lookahead(choice!(python_literal("with"), python_literal("async"))), deferred(with_stmt)),
        seq!(lookahead(choice!(python_literal("for"), python_literal("async"))), deferred(for_stmt)),
        seq!(lookahead(python_literal("try")), deferred(try_stmt)),
        seq!(lookahead(python_literal("while")), deferred(while_stmt)),
        deferred(match_stmt)
    )).into()
}

pub fn simple_stmt() -> Combinator {
    cached(tag("simple_stmt", choice!(
        deferred(assignment),
        seq!(lookahead(python_literal("type")), deferred(type_alias)),
        deferred(star_expressions),
        seq!(lookahead(python_literal("return")), deferred(return_stmt)),
        seq!(lookahead(choice!(python_literal("import"), python_literal("from"))), deferred(import_stmt)),
        seq!(lookahead(python_literal("raise")), deferred(raise_stmt)),
        python_literal("pass"),
        seq!(lookahead(python_literal("del")), deferred(del_stmt)),
        seq!(lookahead(python_literal("yield")), deferred(yield_stmt)),
        seq!(lookahead(python_literal("assert")), deferred(assert_stmt)),
        python_literal("break"),
        python_literal("continue"),
        seq!(lookahead(python_literal("global")), deferred(global_stmt)),
        seq!(lookahead(python_literal("nonlocal")), deferred(nonlocal_stmt))
    ))).into()
}

pub fn simple_stmts() -> Combinator {
    tag("simple_stmts", choice!(
        seq!(deferred(simple_stmt), negative_lookahead(python_literal(";")), deferred(NEWLINE)),
        seq!(seprep1(deferred(simple_stmt), python_literal(";")), opt(python_literal(";")), deferred(NEWLINE))
    )).into()
}

pub fn statement_newline() -> Combinator {
    tag("statement_newline", choice!(
        seq!(deferred(compound_stmt), deferred(NEWLINE)),
        deferred(simple_stmts),
        deferred(NEWLINE),
        deferred(ENDMARKER)
    )).into()
}

pub fn statement() -> Combinator {
    tag("statement", choice!(
        deferred(compound_stmt),
        deferred(simple_stmts)
    )).into()
}

pub fn statements() -> Combinator {
    tag("statements", repeat1(deferred(statement))).into()
}

pub fn func_type() -> Combinator {
    tag("func_type", seq!(
        python_literal("("),
         opt(deferred(type_expressions)),
         python_literal(")"),
         python_literal("->"),
         deferred(expression),
         repeat0(deferred(NEWLINE)),
         deferred(ENDMARKER)
    )).into()
}

pub fn eval() -> Combinator {
    tag("eval", seq!(deferred(expressions), repeat0(deferred(NEWLINE)), deferred(ENDMARKER))).into()
}

pub fn interactive() -> Combinator {
    tag("interactive", deferred(statement_newline)).into()
}

pub fn file() -> Combinator {
    tag("file", seq!(opt(deferred(statements)), deferred(ENDMARKER))).into()
}


pub fn python_file() -> Combinator {

    cache_context(tag("main", seq!(opt(deferred(NEWLINE)), deferred(file)))).compile()
}
