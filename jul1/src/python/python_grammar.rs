...
pub fn python_file() -> Rc<DynCombinator> {
    ...
    let strings = strings.set(tag("strings", seq!(choice!(seq!(&FSTRING_START, opt(seq!(opt(&WS), &fstring_middle, opt(repeat1(seq!(opt(&WS), &fstring_middle))))), opt(&WS), &FSTRING_END), &STRING), opt(repeat1(seq!(opt(&WS), choice!(seq!(&FSTRING_START, opt(seq!(opt(&WS), &fstring_middle, opt(repeat1(seq!(opt(&WS), &fstring_middle))))), opt(&WS), &FSTRING_END), &STRING))))))).into_rc_dyn();
    ...
    let func_type = func_type.set(tag("func_type", seq!(python_literal("("), opt(seq!(opt(&WS), &type_expressions)), opt(&WS), python_literal(")"), opt(&WS), python_literal("->"), opt(&WS), &expression, opt(seq!(opt(&WS), &NEWLINE, opt(repeat1(seq!(opt(&WS), &NEWLINE))))), opt(&WS), &ENDMARKER))).into_rc_dyn();
