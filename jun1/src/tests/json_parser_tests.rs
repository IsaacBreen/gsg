...
mod json_parser {
    use std::rc::Rc;
    use crate::{ActiveCombinator, choice, eat_string, eat_u8, eat_u8_range, eat_u8_range_complement, opt, repeat, seq, Combinator, forward_ref, U8Set, CombinatorState, Choice2};

    #[test]
    fn test_json_parser() {
        // Helper combinators for JSON parsing
        let whitespace = repeat(choice!(eat_u8(' '), eat_u8('\t'), eat_u8('\n'), eat_u8('\r')) as Rc<dyn Combinator<State = Box<dyn CombinatorState>>>);
        ...
        let string = seq!(eat_u8('"'), repeat(string_char as Rc<dyn Combinator<State = Box<dyn CombinatorState>>>), eat_u8('"'));
        ...
        let json_array = Rc::new(seq!(
            ...
            opt(seq!(
                json_value.clone(),
                repeat(seq!(whitespace.clone(), eat_u8(','), whitespace.clone(), json_value.clone()) as Rc<dyn Combinator<State = Box<dyn CombinatorState>>>),
                whitespace.clone(),
            )),
            eat_u8(']'),
        ));
        ...
        let json_object = Rc::new(seq!(
            ...
            opt(seq!(
                ...
                whitespace.clone(),
                repeat(seq!(eat_u8(','), whitespace.clone(), key_value_pair.clone()) as Rc<dyn Combinator<State = Box<dyn CombinatorState>>>),
                whitespace.clone(),
            )),
            eat_u8('}'),
        ));
